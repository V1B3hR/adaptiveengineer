...
class AttackerAgent:
    ...
    def __init__(self, agent_id: int, initial_energy: float = 10.0):
        ...
        logger.info(
            f"AttackerAgent {agent_id} initialized with {len(self.attack_strategies)} strategies"
        )
    ...
    def _initialize_attack_strategies(self) -> None:
        ...
        self.attack_strategies[AttackType.COMMUNICATION_JAMMING] = (
            ThreatPattern(
                signature=[
                    0.3,
                    0.9,
    ...
    def execute_attack(
        self, target_id: int, attack_type: Optional[AttackType] = None
    ...
    def learn_from_detection(
        self, attack_type: AttackType, detected: bool
    ) -> None:
        """
    ...
    def evolve_strategies(self) -> None:
        ...
        for attack_type, pattern in self.attack_strategies.items():
            ...
            detection_rate = self.times_detected / max(
                1, self.successful_attacks + self.failed_attacks
            )
            ...
            if detection_rate > 0.5:
                ...
                logger.info(
                    f"AttackerAgent {self.agent_id} evolved {attack_type.value} strategy"
                )
    ...
    def get_fitness(self) -> float:
        ...
        success_rate = self.successful_attacks / (
            self.successful_attacks + self.failed_attacks
        )
        detection_rate = self.times_detected / (
            self.successful_attacks + self.failed_attacks
        )
...
class AdversarialEnvironment:
    ...
    def __init__(
        ...
        self.defender_ids: Set[int] = (
            set()
        )  # Defender nodes managed externally
        ...
        logger.info(
            f"AdversarialEnvironment initialized: "
    ...
    def simulate_attack_wave(
        self, defender_nodes: Dict[int, Any]
    ...
    def evolve_population(self) -> None:
        ...
        logger.info(
            f"Evolution completed - Generation {self.current_generation}"
        )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        attacker_stats = {
            ...
            "avg_fitness": (
                sum(a.get_fitness() for a in self.attackers.values())
                / len(self.attackers)
                if self.attackers
                else 0.0
            ),
            "total_energy_drained": sum(
                a.total_energy_drained for a in self.attackers.values()
            ),
            "avg_success_rate": (
                sum(
                    a.successful_attacks
                    / max(1, a.successful_attacks + a.failed_attacks)
    ...
    def visualize_arms_race(self) -> str:
        ...
        lines.append(f"\n{'='*70}")
        lines.append(
            f"ADVERSARIAL ARMS RACE - Generation {self.current_generation}"
        )
        lines.append(f"{'='*70}")
        ...
        lines.append(
            f"  Success Rate: {stats['attack_success_rate']*100:.1f}%"
        )
        lines.append(
            f"  Energy Drained: {stats['attacker_stats']['total_energy_drained']:.2f}"
        )
        ...
        lines.append(f"\n🧬 Evolution:")
        lines.append(
            f"  Threat Patterns: {stats['threat_library']['total_patterns']}"
        )
        lines.append(
            f"  Evolution Generations: {stats['threat_library']['evolution_generations']}"
        )
        lines.append(
            f"  Avg Attacker Fitness: {stats['attacker_stats']['avg_fitness']:.3f}"
        )
