...
class AdversaryGenome:
    ...
    def mutate(self, mutation_rate: float = 0.1) -> "AdversaryGenome":
        ...
        if random.random() < mutation_rate:
            mutated.aggression = max(
                0.0, min(1.0, mutated.aggression + random.gauss(0, 0.1))
            )

        if random.random() < mutation_rate:
            mutated.stealth = max(
                0.0, min(1.0, mutated.stealth + random.gauss(0, 0.1))
            )

        if random.random() < mutation_rate:
            mutated.persistence = max(
                0.0, min(1.0, mutated.persistence + random.gauss(0, 0.1))
            )

        if random.random() < mutation_rate:
            mutated.adaptability = max(
                0.0, min(1.0, mutated.adaptability + random.gauss(0, 0.1))
            )
        ...
        for capability in mutated.capabilities:
            if random.random() < mutation_rate:
                capability.success_rate = max(
                    0.0,
                    min(1.0, capability.success_rate + random.gauss(0, 0.05)),
                )

            if random.random() < mutation_rate:
                capability.detection_probability = max(
                    0.0,
                    min(
                        1.0,
                        capability.detection_probability
                        + random.gauss(0, 0.05),
                    ),
                )
...
class EvolvingAdversary:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized {adversary_type} adversary {adversary_id} "
    ...
    def _initialize_genome(self, sophistication: float) -> AdversaryGenome:
        ...
        if self.adversary_type == AdversaryType.APT:
            capabilities = [
                AttackCapability(
                    "spear_phishing", AttackVector.PHISHING, 0.7, 0.3, 0.8, 0.9
                ),
                AttackCapability(
                    "zero_day_exploit",
            ...
            capabilities = [
                ...
                AttackCapability(
                    "exploit_kit", AttackVector.EXPLOIT, 0.7, 0.6, 0.8, 0.7
                ),
                AttackCapability(
                    "credential_theft",
            ...
            capabilities = [
                AttackCapability(
                    "brute_force", AttackVector.BRUTE_FORCE, 0.4, 0.8, 0.3, 0.2
                ),
                AttackCapability(
                    "known_exploit", AttackVector.EXPLOIT, 0.5, 0.7, 0.4, 0.3
                ),
            ]
            ...
            capabilities = [
                AttackCapability(
                    "automated_scan", AttackVector.EXPLOIT, 0.6, 0.6, 0.5, 0.4
                ),
                AttackCapability(
                    "mass_attack", AttackVector.BRUTE_FORCE, 0.5, 0.7, 0.4, 0.3
                ),
            ]
    ...
    def attempt_attack(
        self, target: str, defense_level: float = 0.5
    ) -> AttackAttempt:
        """
        ...
        detection_prob = base_detection * (
            1.0 + defense_bonus - stealth_reduction
        )
        ...
        logger.info(
            f"Adversary {self.adversary_id} attacked {target}: "
    ...
    def _select_capability(self, defense_level: float) -> AttackCapability:
        ...
        if self.genome.adaptability > 0.6:
            recent_blocked = {
                a.capability.name
                for a in list(self.attack_history)[-10:]
                if a.blocked
            }
            ...
            available = [
                c
                for c in self.genome.capabilities
    ...
    def _learn_from_attempt(
        self, attempt: AttackAttempt, defense_level: float
    ):
        """
        ...
        if attempt.success:
            ...
            attempt.capability.success_rate = min(
                1.0, attempt.capability.success_rate + learning_rate
            )

        if attempt.blocked:
            # Reduce reliance on blocked capability
            attempt.capability.success_rate = max(
                0.1, attempt.capability.success_rate - learning_rate
            )
            ...
            if defense_sig not in self.learned_defenses:
                ...
                logger.debug(
                    f"Adversary {self.adversary_id} learned defense: {defense_sig}"
                )

        if attempt.detected and not attempt.blocked:
            # Improve stealth if detected but not blocked
            self.genome.stealth = min(
                1.0, self.genome.stealth + learning_rate * 0.5
            )
    ...
    def _progress_attack_phase(self):
        ...
        if current_idx < len(phase_order) - 1:
            ...
            logger.info(
                f"Adversary {self.adversary_id} progressed to {self.current_phase}"
            )
    ...
    def evolve(self, mutation_rate: float = 0.1) -> "EvolvingAdversary":
        ...
        logger.info(
            f"Adversary {self.adversary_id} evolved to {new_id} "
    ...
    def calculate_fitness(self) -> float:
        ...
        # Weighted fitness
        fitness = (
            success_rate * 0.4 + evasion_rate * 0.3 + persistence_rate * 0.3
        )
...
class AdversarySimulation:
    ...
    def __init__(self, population_size: int = 5):
        ...
        logger.info(
            f"Adversary simulation initialized with {population_size} adversaries"
        )
    ...
    def _initialize_population(self):
        ...
        for i in range(self.population_size):
            ...
            adversary = EvolvingAdversary(
                adversary_id, adv_type, sophistication
            )
            self.adversaries[adversary_id] = adversary

    def simulate_attacks(
        self, targets: List[str], defense_level: float, rounds: int = 10
    ) -> List[AttackAttempt]:
        """
    ...
    def evolve_population(self, selection_rate: float = 0.5):
        ...
        sorted_adversaries = sorted(
            self.adversaries.values(), key=lambda a: a.fitness, reverse=True
        )
        ...
        logger.info(
            f"Evolved adversary population to generation {self.generation}"
        )

    def get_simulation_metrics(self) -> Dict[str, Any]:
        """Get overall simulation metrics"""
        total_attacks = sum(
            a.attacks_attempted for a in self.adversaries.values()
        )
        total_successful = sum(
            a.attacks_successful for a in self.adversaries.values()
        )

        avg_sophistication = sum(
            a.sophistication for a in self.adversaries.values()
        ) / len(self.adversaries)
        avg_fitness = sum(a.fitness for a in self.adversaries.values()) / len(
            self.adversaries
        )
        ...
        return {
            ...
            "adversaries": [
                a.get_metrics() for a in self.adversaries.values()
            ],
        }
