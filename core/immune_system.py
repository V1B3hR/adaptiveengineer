...
class ThreatSignature:
    ...
    def update_occurrence(self, severity: float) -> None:
        ...
        self.severity = (
            self.severity * (self.occurrence_count - 1) + severity
...
class NeutrophilAgent:
    ...
    def respond(
        self, threat_node: str, threat_level: ThreatLevel
    ) -> Dict[str, Any]:
        """
        ...
        actions = ["isolate_node", "block_incoming_traffic", "prevent_spread"]

        # Higher threat levels require more aggressive containment
        if threat_level in [ThreatLevel.HIGH, ThreatLevel.CRITICAL]:
            actions.extend(["quarantine_neighbors", "suspend_services"])

        success_probability = (
            0.9 if threat_level != ThreatLevel.CRITICAL else 0.7
        )
    ...
    def respond(
        ...
        if self.knowledge_graph and pattern_id:
            ...
            if best_strategies:
                ...
                strategy = self.knowledge_graph.solution_strategies.get(
                    strategy_id
                )
...
class ReinforcementSquad:
    ...
    def respond(
        self, stress_node: str, stress_metrics: Dict[str, float]
    ) -> Dict[str, Any]:
        """
...
class BCellAgent:
    ...
    def recognize_threat(
        self, observed_features: Dict[str, Any]
    ) -> Tuple[bool, float]:
        """
...
class AdaptiveImmuneSystem:
    ...
    def initialize_agents(
        ...
        for i in range(macrophage_count):
            self.macrophages.append(
                MacrophageAgent(f"macrophage_{i}", self.knowledge_graph)
            )
        ...
        logger.info(
            f"Initialized immune system: {neutrophil_count} neutrophils, "
    ...
    def detect_threat(
        ...
        for b_cell in self.b_cells:
            ...
            if is_match:
                logger.info(
                    f"Memory cell {b_cell.agent_id} recognized threat "
    ...
    def initiate_response(
        ...
        response_id = (
            f"response_{int(time.time() * 1000)}_{self.total_responses}"
        )
        ...
        logger.info(
            f"Initiated immune response {response_id} for {target_node}"
        )
    ...
    def _deploy_neutrophils(self, response: ImmuneResponse) -> None:
        ...
        for neutrophil in available[:count]:
            result = neutrophil.respond(
                response.target_node, response.threat_level
            )
    ...
    def escalate_to_repair(
        self, response_id: str, pattern_id: Optional[str] = None
    ) -> None:
        """
        ...
        logger.info(
            f"Escalated {response_id} to repair phase with {macrophage.agent_id}"
        )

    def deploy_reinforcements(
        self, target_node: str, stress_metrics: Dict[str, float]
    ) -> str:
        """
        ...
        available = [
            s for s in self.reinforcement_squads if s.current_location is None
        ]
        ...
        logger.info(
            f"Deployed reinforcement squad {squad.squad_id} to {target_node}"
        )
    ...
    def complete_response(self, response_id: str, success: bool) -> None:
        ...
        response.status = (
            ResponseStatus.RESOLVED if success else ResponseStatus.FAILED
        )
        ...
        logger.info(
            f"Response {response_id} completed in {duration:.1f}s "
    ...
    def _create_memory_cell(self, threat_signature: ThreatSignature) -> None:
        ...
        logger.info(
            f"Created memory cell {b_cell_id} for {threat_signature.threat_type}"
        )
    ...
    def _calculate_avg_response_time(self) -> float:
        ...
        total_time = sum(
            r.get_response_duration() for r in self.completed_responses
        )
