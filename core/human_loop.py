...
class HumanLoopSystem:
    ...
    def request_approval(
        ...
        # Auto-approve low sensitivity if configured
            ...
            self.auto_approve_low
            and sensitivity == ActionSensitivity.LOW
            and risk_level < 0.3
        ):
            request.status = ApprovalStatus.AUTO_APPROVED
    ...
    def set_override_rule(self, action: str, allow: bool):
        ...
        logger.info(
            f"Override rule set: {action} -> {'allow' if allow else 'deny'}"
        )
    ...
    def check_compliance(
        ...
        for framework in self.compliance_frameworks:
            compliant = self._check_framework_compliance(
    ...
    def _check_framework_compliance(
        ...
        if framework == ComplianceFramework.GDPR:
            ...
            if context.get("accesses_pii"):
                ...
                return context.get("has_consent", False) and context.get(
                    "has_purpose", False
                )
            return True
            ...
            if context.get("accesses_phi"):
                return context.get("has_authorization", False) and context.get(
                    "is_encrypted", False
                )
            return True
            ...
            return context.get("is_audited", True) and context.get(
                "is_secure", True
            )
            ...
            if context.get("accesses_payment_data"):
                return context.get("is_encrypted", False) and context.get(
                    "is_logged", False
                )
            return True
    ...
    def get_human_loop_metrics(self) -> Dict[str, Any]:
        ...
        approval_rate = (
            ...
            if self.approval_requests_made > 0
            else 0.0
        )
