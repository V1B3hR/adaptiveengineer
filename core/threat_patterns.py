...
class ThreatPattern:
    ...
    attack_type: str = (
        "unknown"  # energy_drain, jamming, trust_poisoning, etc.
    )
    ...
    def add_countermeasure(
        self, countermeasure: str, effectiveness: float
    ) -> None:
        """
...
class ThreatLibrary:
    ...
    def __init__(self, similarity_threshold: float = 0.7):
        ...
        self.pattern_lineage: Dict[str, List[str]] = (
            {}
        ...
        logger.info(
            f"ThreatLibrary initialized with similarity threshold {similarity_threshold}"
        )
    ...
    def add_pattern(self, pattern: ThreatPattern) -> None:
        ...
        logger.info(
            f"Added threat pattern {pattern.pattern_id}: "
            f"type={pattern.attack_type}, severity={pattern.severity:.2f}"
        )

    def find_similar(
        self, pattern: ThreatPattern, threshold: Optional[float] = None
    ...
    def _calculate_similarity(
        self, pattern1: ThreatPattern, pattern2: ThreatPattern
    ) -> float:
        """
        ...
        normalized_distance = (
            distance / max_distance if max_distance > 0 else 0.0
        )
        ...
        type_match = (
            1.0 if pattern1.attack_type == pattern2.attack_type else 0.7
        )
    ...
    def evolve_patterns(self, count: int = 5) -> List[ThreatPattern]:
        ...
        patterns_list.sort(
            key=lambda p: (
                p.failed_mitigations
                / max(1, p.successful_mitigations + p.failed_mitigations)
            ),
            reverse=True,
        )
        ...
        logger.info(
            f"Evolved {len(evolved)} threat patterns (generation {self.evolution_generations})"
        )

        return evolved

    def get_effective_countermeasures(
        self, pattern: ThreatPattern, top_n: int = 5
    ) -> List[Tuple[str, float]]:
        """
        ...
        for similar, similarity in similar_patterns[:3]:  # Top 3 similar
            for cm, effectiveness in similar.effectiveness_history.items():
                ...
                countermeasures[cm].extend(
                    [e * similarity for e in effectiveness]
                )
    ...
    def get_patterns_by_type(self, attack_type: str) -> List[ThreatPattern]:
        ...
        return [
            p for p in self.patterns.values() if p.attack_type == attack_type
        ]
