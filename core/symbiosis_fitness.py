...
def evaluate_symbiosis(strategy, scenario_data: List[Any]) -> float:
    ...
    expected_value = (
        scenario_data[0].value if hasattr(scenario_data[0], "value") else 10.0
    )
    ...
    for i, obs in enumerate(scenario_data):
        ...
        if error < 2.0:  # Low error = stable environment
            joy += (
                joy_gain * calmness
            )  # Calm agents gain more joy from stability
            ...
            stress += (
                (1.0 - calmness) * error * 0.05
            )  # Non-calm agents get stressed
    ...
    fitness = max(
        0.0, min(1.0, avg_joy - (avg_stress * 0.5) + (avg_adaptation * 2.0))
    )
