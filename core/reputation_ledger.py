...
class PrivilegeTier(Enum):
    ...
    TRUSTED_PEER = 1 # Can join collaborative tasks, signals considered in decisions
    VETERAN = 2      # Can lead squads, request higher resources, use advanced sensors
...
class ReputationRecord:
    ...
    agent_id: str
    timestamp: float
    action_type: str  # e.g., "task_completion", "incident_resolution", "reliable_info"
...
class AgentReputation:
    ...
    def has_capability(self, capability: str) -> bool:
        ...
        tier_capabilities = {
            ...
            PrivilegeTier.VETERAN: {"sense", "move", "join_task", "vote", "lead_squad", 
                                   "request_resources", "advanced_sensors"},
            PrivilegeTier.EMERITUS: {"sense", "move", "join_task", "vote", "lead_squad",
                                    "request_resources", "advanced_sensors", "propose_strategy"}
        }
...
class ReputationLedger:
    ...
    def __init__(self, 
        ...
        logger.info(f"Reputation ledger initialized (byzantine_tolerance={byzantine_tolerance})")
    ...
    def register_agent(self, agent_id: str) -> AgentReputation:
        ...
        logger.info(f"Agent {agent_id} registered with tier {reputation.privilege_tier.name}")
        return reputation
    ...
    def remove_validator(self, validator_id: str):
        ...
        if validator_id in self.validators:
            ...
            logger.warning(f"Validator {validator_id} removed from reputation ledger")
    
    def record_reputation_change(self,
                                 agent_id: str,
                                 amount: float,
                                 action_type: str,
                                 validator_id: Optional[str] = None,
                                 requires_consensus: bool = False) -> bool:
        """
        ...
        if validator_id:
            if validator_id not in self.validators:
                logger.warning(f"Unknown validator {validator_id}, cannot validate")
                return False
            
            if validator_id in self.suspicious_validators:
                logger.warning(f"Suspicious validator {validator_id}, rejecting")
                return False
        ...
        if requires_consensus and len(self.validators) > 0:
            required_validations = int(len(self.validators) * (1 - self.byzantine_tolerance)) + 1
        ...
        logger.info(f"Reputation change: {agent_id} {amount:+.1f} ({action_type}) "
    ...
    def get_top_agents(self, limit: int = 10) -> List[Tuple[str, float]]:
        ...
        return [(agent_id, agent.total_reputation) 
    ...
    def detect_byzantine_behavior(self, agent_id: str) -> bool:
        ...
        if len(recent_records) >= 10:
            ...
            if time_span < 60.0:  # 10 changes in less than 1 minute
                logger.warning(f"Rapid reputation changes detected for {agent_id}")
                return True
            ...
            for validator_id, count in validator_counts.items():
                if count > 7:  # More than 70% from one validator
                    logger.warning(f"Suspicious validator pattern for {agent_id} "
    ...
    def verify_ledger_integrity(self) -> bool:
        ...
        """
        logger.info(f"Verifying ledger integrity ({len(self.records)} records)...")
        
        for i, record in enumerate(self.records):
            # Verify signature
            if not record.verify_signature(self.secret_key):
                logger.error(f"Invalid signature at record {i}: {record.agent_id}")
                return False
        ...
        for record in self.records:
            computed_reputations[record.agent_id] = \
                computed_reputations.get(record.agent_id, 0.0) + record.reputation_change
        
        for agent_id, computed_rep in computed_reputations.items():
            agent = self.agents.get(agent_id)
            if agent and abs(agent.total_reputation - computed_rep) > 0.01:
                logger.error(f"Reputation mismatch for {agent_id}: "
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            'avg_reputation': sum(a.total_reputation for a in self.agents.values()) / len(self.agents) if self.agents else 0.0
        }
