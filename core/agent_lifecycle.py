...
class BirthConditions:
    ...
    min_energy_available: float = 5.0  # Environment must have sufficient energy
    min_health_threshold: float = 0.3  # Spawning location must be healthy
    max_stress_threshold: float = 0.7  # Spawning location can't be too stressed
...
class DeathConditions:
    ...
    zero_energy_threshold: float = 0.0  # Energy at or below this triggers death
    zero_health_threshold: float = 0.0  # Health at or below this triggers death
    critical_stress_threshold: float = 1.0  # Stress at or above this can trigger death
    max_lifespan: Optional[float] = None  # Maximum age in seconds (None = unlimited)
    death_probability_low_energy: float = 0.01  # Chance of death per step when energy is low
    death_probability_high_stress: float = 0.005  # Chance of death per step when stress is high
...
class AgentLifecycleManager:
    ...
    def __init__(self, birth_conditions: Optional[BirthConditions] = None,
    ...
    def check_birth_conditions(self, environment=None, spawn_location_id: Optional[str] = None) -> bool:
        """
        ...
        if current_time - self.last_spawn_time < self.birth_conditions.spawn_cooldown:
        ...
        if environment and spawn_location_id:
            ...
            if location_state:
                ...
                if location_state.get('energy_provided', 0.0) < self.birth_conditions.min_energy_available:
                    logger.debug(f"Birth blocked: insufficient energy at location {spawn_location_id}")
                    return False
                
                # Check location health
                health_status = location_state.get('health_status', 'unknown')
                if health_status == 'failed':
                    logger.debug(f"Birth blocked: failed health status at location {spawn_location_id}")
                    return False
                
                # Check stress level
                if location_state.get('stress_level', 0.0) > self.birth_conditions.max_stress_threshold:
                    logger.debug(f"Birth blocked: high stress at location {spawn_location_id}")
                    return False
                
                # Check if location can host another agent
                if not location_state.get('can_host_agent', False):
                    logger.debug(f"Birth blocked: location {spawn_location_id} cannot host more agents")
                    return False
        
        return True
    
    def spawn_agent(self, agent_class, spawn_location_id: Optional[str] = None,
                   initial_energy: float = 10.0, **kwargs) -> Optional[Any]:
        """
        ...
        """
        if not self.check_birth_conditions(kwargs.get('environment'), spawn_location_id):
            return None
        ...
        try:
            agent = agent_class(
                ...
                node_id=agent_id,
                **{k: v for k, v in kwargs.items() if k not in ['position', 'velocity', 'initial_energy', 'environment']}
            )
            ...
            logger.info(f"Agent {agent_id} born at location {spawn_location_id}")
    ...
    def check_death_conditions(self, agent) -> tuple[bool, Optional[DeathCause]]:
        """
        ...
        if hasattr(agent, 'energy') and agent.energy <= self.death_conditions.zero_energy_threshold:
            logger.info(f"Agent {agent.node_id} death condition: energy depletion ({agent.energy:.2f})")
        ...
        if hasattr(agent, 'health'):
            if agent.health <= self.death_conditions.zero_health_threshold:
                logger.info(f"Agent {agent.node_id} death condition: health failure ({agent.health:.2f})")
                return True, DeathCause.HEALTH_FAILURE
        
        # Check stress overload
        if hasattr(agent, 'anxiety') or hasattr(agent, 'stress_level'):
            stress = getattr(agent, 'stress_level', getattr(agent, 'anxiety', 0.0))
            if stress >= self.death_conditions.critical_stress_threshold:
                # Probabilistic death from high stress
                import numpy as np
                if np.random.random() < self.death_conditions.death_probability_high_stress:
                    logger.info(f"Agent {agent.node_id} death condition: stress overload ({stress:.2f})")
                    return True, DeathCause.STRESS_OVERLOAD
        
        # Check low energy (probabilistic)
        if hasattr(agent, 'energy') and agent.energy < 2.0:
            import numpy as np
            if np.random.random() < self.death_conditions.death_probability_low_energy:
                logger.info(f"Agent {agent.node_id} death condition: low energy ({agent.energy:.2f})")
                return True, DeathCause.ENERGY_DEPLETION
        ...
        if self.death_conditions.max_lifespan is not None:
            if hasattr(agent, '_time'):
                ...
                if age >= self.death_conditions.max_lifespan:
                    logger.info(f"Agent {agent.node_id} death condition: natural expiration (age {age:.0f})")
    ...
    def process_agent_death(self, agent, cause: DeathCause):
        ...
        death_event = {
            ...
            "final_health": getattr(agent, 'health', getattr(agent, 'energy', 0.0)),
            "age": getattr(agent, '_time', 0.0),
            "position": tuple(agent.position) if hasattr(agent, 'position') else None,
            "memory_count": len(agent.memory) if hasattr(agent, 'memory') else 0,
            "trust_network_size": len(agent.trust_network) if hasattr(agent, 'trust_network') else 0
        }
    ...
    def get_population_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "birth_rate": self.total_births / max(1, time.time() - (self.birth_history[0]["timestamp"] if self.birth_history else time.time())),
            "death_rate": self.total_deaths / max(1, time.time() - (self.dead_agents[0]["timestamp"] if self.dead_agents else time.time()))
        }
    ...
    def force_agent_death(self, agent_id: int, cause: DeathCause = DeathCause.EXTERNAL_TERMINATION):
        """
