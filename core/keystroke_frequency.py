...
class KeystrokeFrequencyAnalyzer:
    ...
    def __init__(
        ...
        logger.info(
            f"Keystroke Analyzer initialized: min_samples={min_samples_for_baseline}, "
            f"threshold={authentication_threshold}"
        )

    def add_keystroke(self, event: KeystrokeEvent):
        """
    ...
    def establish_baseline(
        self, user_id: str, keystrokes: Optional[List[KeystrokeEvent]] = None
        ...
        if len(keystroke_list) < self.min_samples_for_baseline:
            logger.info(
                f"Insufficient keystrokes for {user_id}: {len(keystroke_list)}/{self.min_samples_for_baseline}"
            )
            return None
        ...
        for i in range(len(keystroke_list) - 1):
            flight_time = keystroke_list[i].flight_time_to(
                keystroke_list[i + 1]
            )
        ...
        fft_coeffs = self._compute_typing_fft_signature(
        ...
        logger.info(
            f"Established baseline for {user_id}: "
    ...
    def _compute_typing_fft_signature(
        self, dwell_times: List[float], flight_times: List[float]
    ) -> np.ndarray:
        """
        ...
        if len(signature) < self.fft_coefficients:
            signature = np.pad(
                signature, (0, self.fft_coefficients - len(signature))
            )
    ...
    def authenticate(
        ...
        current_fft = self._compute_typing_fft_signature(
            dwell_times, flight_times
        )
        ...
        dwell_distance = abs(
            current_mean_dwell - baseline.mean_dwell_time
        ) / max(baseline.std_dwell_time, MIN_STD_THRESHOLD)
        flight_distance = abs(
            current_mean_flight - baseline.mean_flight_time
        ) / max(baseline.std_flight_time, MIN_STD_THRESHOLD)

        # Combined distance (weighted)
        combined_distance = (
            0.6 * fft_distance + 0.2 * dwell_distance + 0.2 * flight_distance
        )
        ...
        if combined_distance < self.authentication_threshold:
            ...
            confidence = 1.0 - (
                combined_distance / self.authentication_threshold
            )
            ...
            confidence = (
                0.5
                - (combined_distance / (self.authentication_threshold * 2))
                * 0.5
            )
        ...
        if status == AuthenticationStatus.REJECTED:
            logger.warning(
                f"Authentication rejected for {user_id}: distance={combined_distance:.3f}"
            )
        elif status == AuthenticationStatus.SUSPICIOUS:
            logger.info(
                f"Suspicious authentication for {user_id}: distance={combined_distance:.3f}"
            )
    ...
    def detect_impersonation(
        ...
        if continuous_monitoring:
            ...
            for i in range(
                0, len(keystrokes) - window_size + 1, window_size // 2
            ):
                window = keystrokes[i : i + window_size]
                result = self.authenticate(
                    user_id, window, min_keystrokes=window_size
                )
                results.append(result)
        else:
            # Single authentication on recent keystrokes
            result = self.authenticate(
                user_id, keystrokes[-window_size:], min_keystrokes=window_size
            )
            results.append(result)
    ...
    def simulate_typing(
        ...
        for rep in range(num_repetitions):
            for char in text + " ":  # Add space between repetitions
                ...
                if add_noise:
                    dwell = (
                        base_dwell + user_factor + np.random.normal(0, 0.02)
                    )
                else:
    ...
    def get_user_baseline(self, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "fft_signature_norm": float(
                np.linalg.norm(signature.fft_coefficients)
            ),
        }
