...
class GossipMessage:
    ...
    gossip_type: str  # e.g., "threat_signature", "trust_update", "strategy_change"
    ...
    def should_propagate(self, node_id: int) -> bool:
        ...
        return (
            node_id not in self.seen_by and
            self.ttl > 0 and
            self.generation < 20  # Limit propagation depth
        )
...
class CryptoSigner:
    ...
    def _generate_secret(self) -> str:
        ...
        return hashlib.sha256(f"{self.node_id}_{time.time()}_{uuid.uuid4()}".encode()).hexdigest()
...
class AdvancedSensorySystem:
    ...
    def proprioception(self) -> Dict[str, Any]:
        ...
        self.last_proprioception = {
            ...
            "computational_load": getattr(self.agent, 'communications_this_step', 0) / 
            ...
            "position": tuple(self.agent.position) if hasattr(self.agent, 'position') else None,
            "timestamp": time.time()
        }
        return self.last_proprioception
    
    def local_environmental_sensing(self, graph_env, current_node_id: str) -> Dict[str, Any]:
        """
        ...
        neighbor_states = [graph_env.get_node_state(nid) for nid in neighbors[:5]]  # Limit to 5
        ...
        self.last_local_sense = {
            ...
            "local_threat_level": node_state.get('active_threat_score', 0.0) if node_state else 0.0,
            "local_resources": {
                "energy_available": node_state.get('energy_provided', 0.0) if node_state else 0.0,
                "cpu_load": node_state.get('cpu_load', 0.0) if node_state else 0.0,
                "stress_level": node_state.get('stress_level', 0.0) if node_state else 0.0
            },
            "timestamp": time.time()
        }
        return self.last_local_sense
    
    def graph_level_awareness(self, graph_env, query_type: str, 
        ...
        if self.agent.energy < self.query_energy_cost:
            logger.debug(f"Node {self.agent.node_id}: Insufficient energy for graph-level query")
            return None
    ...
    def get_full_sensory_state(self, graph_env=None, current_node_id: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        if graph_env and current_node_id:
            state["local"] = self.local_environmental_sensing(graph_env, current_node_id)
...
class AdvancedCommunicationProtocol:
    ...
    def __init__(self, agent_node):
        ...
        # Pheromone tracking
        self.deposited_pheromones: Dict[str, List[str]] = defaultdict(list)  # node_id -> pheromone_ids
        ...
        # Gossip protocol
        self.gossip_cache: Dict[str, GossipMessage] = {}  # gossip_id -> message
        self.gossip_inbox: deque = deque(maxlen=100)
        self.gossip_propagation_rate = 0.3  # Probability of propagating a gossip message
    ...
    def deposit_pheromone(self, node_id: str, content: str, 
                         intensity: float = 1.0, decay_rate: float = 0.95) -> str:
        """
        ...
        logger.debug(f"Node {self.agent.node_id}: Deposited pheromone on node {node_id}")
        
        return pheromone_id
    
    def sense_pheromones(self, node_id: str, pheromone_store: Dict[str, List[Pheromone]]) -> List[Dict[str, Any]]:
        """
        ...
        return [p.to_dict() for p in pheromone_store[node_id] if not p.is_expired()]
    
    # ========== Signal Methods ==========
    
    def send_signal(self, recipient_ids: List[int], message_type: str,
        ...
        message_str = f"{signal_id}:{self.agent.node_id}:{message_type}:{str(payload)}"
        ...
        logger.debug(f"Node {self.agent.node_id}: Sent signal {signal_id} to {recipient_ids}")
    ...
    def receive_signal(self, signal: Signal) -> bool:
        ...
        logger.debug(f"Node {self.agent.node_id}: Received signal {signal.signal_id} from {signal.sender_id}")
    ...
    def initiate_gossip(self, gossip_type: str, content: Dict[str, Any]) -> str:
        """
        ...
        # Sign the gossip
        message_str = f"{gossip_id}:{self.agent.node_id}:{gossip_type}:{str(content)}"
        signature = self.crypto_signer.sign(message_str)
        ...
        logger.debug(f"Node {self.agent.node_id}: Initiated gossip {gossip_id} ({gossip_type})")
    ...
    def receive_gossip(self, gossip: GossipMessage) -> bool:
        ...
        logger.debug(f"Node {self.agent.node_id}: Received gossip {gossip.gossip_id} (gen {gossip.generation})")
    ...
    def propagate_gossip(self, nearby_agents: List[Any]) -> int:
        ...
        for gossip in list(self.gossip_cache.values()):
            ...
            num_neighbors = max(1, int(len(nearby_agents) * 0.3))  # 30% of neighbors
            selected = np.random.choice(nearby_agents, size=min(num_neighbors, len(nearby_agents)), replace=False)
            ...
            for agent in selected:
                if hasattr(agent, 'communication_protocol'):
                    if agent.communication_protocol.receive_gossip(gossip_copy):
                        propagated += 1
            
            self.metrics["gossip_propagated"] += propagated
        
        if propagated > 0:
            logger.debug(f"Node {self.agent.node_id}: Propagated {propagated} gossip messages")
    ...
    def cleanup_old_gossip(self, max_age: float = 600.0):
        ...
        if to_remove:
            logger.debug(f"Node {self.agent.node_id}: Cleaned up {len(to_remove)} old gossip messages")
