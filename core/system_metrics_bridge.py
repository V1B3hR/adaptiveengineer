...
class SystemMetricsBridge:
    ...
    def __init__(
        self, energy_scale: float = 10.0, position_to_network: bool = True
    ):
        """
    ...
    def calibrate(self, samples: int = 10, interval: float = 1.0) -> None:
        ...
        if self._calibration_samples:
            self.baseline_metrics = SystemMetrics(
                timestamp=time.time(),
                cpu_percent=sum(
                    m.cpu_percent for m in self._calibration_samples
                )
                / len(self._calibration_samples),
                memory_percent=sum(
                    m.memory_percent for m in self._calibration_samples
                )
                / len(self._calibration_samples),
                ...
                battery_percent=(
                    sum(
                        m.battery_percent
                        for m in self._calibration_samples
            ...
            logger.info(
                f"Baseline established: CPU={self.baseline_metrics.cpu_percent:.1f}%, "
    ...
    def collect_system_metrics(self) -> SystemMetrics:
        ...
        if self._last_network_counters:
            net_bytes_sent = (
                net_io.bytes_sent - self._last_network_counters.bytes_sent
            )
            net_bytes_recv = (
                net_io.bytes_recv - self._last_network_counters.bytes_recv
            )
        else:
            net_bytes_sent = 0
            net_bytes_recv = 0
        ...
        if disk_io:
            if self._last_disk_counters:
                disk_read = (
                    disk_io.read_bytes - self._last_disk_counters.read_bytes
                )
                disk_write = (
                    disk_io.write_bytes - self._last_disk_counters.write_bytes
                )
            else:
                disk_read = 0
                disk_write = 0
    ...
    def map_to_simulation(self, metrics: SystemMetrics) -> Dict[str, float]:
        ...
        if metrics.battery_percent is not None:
            ...
            energy_normalized = (
                0.5 * energy_from_cpu + 0.5 * energy_from_memory
            )
        ...
        processing_activity = min(
            1.0, total_disk / (10 * 1024 * 1024)
        )  # 10MB/s = high
    ...
    def map_from_simulation(
        ...
        expected_network_rate = (
            sim_state.communication_load * 1024 * 1024
        )  # Bytes/s
        ...
        return {
            ...
            "expected_battery_drain": (1.0 - energy_ratio)
            * 10.0,  # % per hour
        }
    ...
    def detect_anomaly(
        ...
        # Check CPU
        cpu_diff = abs(
            current_metrics.cpu_percent - expected["expected_cpu_percent"]
        )
        if cpu_diff > threshold * 100:
            anomalies.append(
                f"CPU: actual={current_metrics.cpu_percent:.1f}%, "
        ...
        mem_diff = abs(
            ...
            - expected["expected_memory_percent"]
        )
        if mem_diff > threshold * 100:
            anomalies.append(
                f"Memory: actual={current_metrics.memory_percent:.1f}%, "
        ...
        if current_network > 0:
            ...
            if net_diff > threshold * expected["expected_network_rate"]:
                anomalies.append(
                    f"Network: actual={current_network} B/s, "
                    f"expected={expected['expected_network_rate']:.0f} B/s"
                )

        return len(anomalies) > 0, anomalies

    def apply_defensive_action(
        self, action: str, parameters: Optional[Dict] = None
    ) -> bool:
        """
                ...
                try:
                    if (
                        ...
                        if current_nice < 10:  # Don't over-throttle
                            ...
                            logger.info(
                                f"Throttled process {proc.info['name']} (PID {proc.info['pid']})"
                            )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        total_network = sum(
            m.network_bytes_sent + m.network_bytes_recv for m in recent_metrics
        )
        ...
        return {
            ...
            "battery_available": any(
                m.battery_percent is not None for m in recent_metrics
            ),
        }
