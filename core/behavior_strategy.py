...
class BehaviorNode:
    ...
    def execute(self, context: Dict[str, Any]) -> BehaviorStatus:
        ...
        if self.node_type == NodeType.SEQUENCE:
            ...
            # Evaluate condition
            if self.condition_fn and self.condition_fn in context.get("conditions", {}):
                result = context["conditions"][self.condition_fn](context, self.parameters)
            ...
            if self.action_fn and self.action_fn in context.get("actions", {}):
                result = context["actions"][self.action_fn](context, self.parameters)
                self.status = BehaviorStatus.SUCCESS if result else BehaviorStatus.FAILURE
            else:
                self.status = BehaviorStatus.FAILURE
            return self.status
...
class FiniteStateMachine:
    ...
    state_actions: Dict[StateType, str] = field(default_factory=dict)  # State -> action name
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "state_actions": {k.value: v for k, v in self.state_actions.items()}
        }
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'FiniteStateMachine':
        return cls(
            ...
            transitions=[FSMTransition.from_dict(t) for t in data.get("transitions", [])],
            state_actions={StateType(k): v for k, v in data.get("state_actions", {}).items()}
        )
    ...
    def update(self, context: Dict[str, Any]) -> Tuple[StateType, bool]:
        ...
        for transition in valid_transitions:
            if transition.condition in context.get("conditions", {}):
                if context["conditions"][transition.condition](context, transition.parameters):
                    # Transition is valid
...
class BehaviorStrategy:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "behavior_tree": self.behavior_tree.to_dict() if self.behavior_tree else None,
...
class BehaviorStrategyFactory:
    ...
    def create_scout_strategy() -> BehaviorStrategy:
        ...
        fsm = FiniteStateMachine(
            ...
            transitions=[
                FSMTransition(StateType.PATROL, StateType.ANALYZE, "detect_threat", 0.9),
                FSMTransition(StateType.ANALYZE, StateType.PATROL, "threat_cleared", 0.7),
                FSMTransition(StateType.PATROL, StateType.IDLE, "low_energy", 0.8)
            ],
            state_actions={
    ...
    def create_harvester_strategy() -> BehaviorStrategy:
        ...
        fsm = FiniteStateMachine(
            ...
            transitions=[
                FSMTransition(StateType.GATHER, StateType.IDLE, "inventory_full", 0.9),
                FSMTransition(StateType.IDLE, StateType.GATHER, "inventory_empty", 0.8)
            ],
            state_actions={
    ...
    def create_guardian_strategy() -> BehaviorStrategy:
        ...
        fsm = FiniteStateMachine(
            ...
            current_state=StateType.PATROL,
            transitions=[
                FSMTransition(StateType.PATROL, StateType.DEFEND, "threat_detected", 0.95),
                FSMTransition(StateType.DEFEND, StateType.PATROL, "threat_neutralized", 0.9),
                FSMTransition(StateType.DEFEND, StateType.RETREAT, "overwhelmed", 0.85)
            ],
            state_actions={
