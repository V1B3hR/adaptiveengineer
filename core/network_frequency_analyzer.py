...
class NetworkFrequencyAnalyzer:
    ...
    def __init__(
        ...
        logger.info(
            f"Network Frequency Analyzer initialized: window={window_size}, "
    ...
    def add_network_event(self, event: NetworkEvent):
        ...
        if event.source_ip not in self.ip_event_buffers:
            self.ip_event_buffers[event.source_ip] = deque(
    ...
    def analyze_traffic_periodicity(
        ...
        freqs, power_spectrum = self._compute_fft_interarrival(
            inter_arrival_times
        )
        ...
        for peak_freq, peak_power, period in peaks:
            ...
            pattern_type, threat_level = self._classify_traffic_pattern(
                period, event_list
            )
            ...
            if confidence > 0.3:  # Only report if reasonably confident
                threat = NetworkThreat(
                    ...
                    affected_ips=(
                        [source_ip]
                        if source_ip
                        else list(set(e.source_ip for e in event_list))
                    ),
                    metadata={
                ...
                if threat_level not in [ThreatLevel.NONE, ThreatLevel.LOW]:
                    logger.warning(
                        f"Network threat: {pattern_type.value} from "
    ...
    def _compute_fft_interarrival(
        self, inter_arrival_times: np.ndarray
    ...
    def _classify_traffic_pattern(
        self, period: float, events: List[NetworkEvent]
    ...
    def detect_c2_beaconing(
        self, source_ip: Optional[str] = None, min_beacons: int = 5
    ) -> List[NetworkThreat]:
        """
        ...
        if source_ip:
            ips_to_check = (
                [source_ip] if source_ip in self.ip_event_buffers else []
            )
        else:
        ...
        for ip in ips_to_check:
            ...
            if (
                30 <= mean_interval <= 120
                and std_interval / mean_interval < 0.2
            ):
                ...
                if unique_dests <= 3:  # Connecting to same few hosts
                    ...
                    logger.critical(
                        f"C2 beacon detected from {ip}: {mean_interval:.1f}s period, "
    ...
    def detect_port_scanning(
        self, source_ip: Optional[str] = None, min_ports: int = 20
    ) -> List[NetworkThreat]:
        """
        ...
        for ip in ips_to_check:
            ...
            if len(unique_ports) >= min_ports:
                ...
                if len(events) > 1:
                    ...
                    scan_rate = (
                        len(unique_ports) / duration if duration > 0 else 0
                    )
                    ...
                    threats.append(threat)
                    logger.warning(
                        f"Port scan detected from {ip}: {len(unique_ports)} ports, "
    ...
    def detect_covert_timing_channel(
        self, events: Optional[List[NetworkEvent]] = None
    ) -> List[NetworkThreat]:
        """
        ...
        if std_interval > 0:
            ...
            if (
                ...
                threats.append(threat)
                logger.warning(
                    f"Covert timing channel detected: {mean_interval*1000:.2f}ms intervals"
                )
