...
class Incident:
    ...
    def should_expire(
        self, current_time: float, default_retention_days: int = 90
    ) -> bool:
        """
...
class Pattern:
    ...
    def update_from_incident(self, incident: Incident) -> None:
        ...
        self.severity_avg = (
            self.severity_avg * (self.occurrence_count - 1) + incident.severity
...
class InMemoryIncidentStore(IncidentStore):
    ...
    def __init__(self, default_retention_days: int = 90):
        ...
        logger.info(
            f"InMemoryIncidentStore initialized (retention: {default_retention_days} days)"
        )
    ...
    def store_incident(self, incident: Incident) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Stored incident {incident.incident_id} ({incident.incident_type.value})"
            )
            return True
    ...
    def query_incidents(
        ...
        if incident_types is not None:
            incidents = [
                i for i in incidents if i.incident_type in incident_types
            ]
        if sources is not None:
            incidents = [i for i in incidents if i.source in sources]
        if tags is not None:
            incidents = [
                i for i in incidents if any(tag in i.tags for tag in tags)
            ]
    ...
    def clear_expired(self, current_time: float) -> int:
        ...
        with self._lock:
            expired = [
                ...
                if incident.should_expire(
                    current_time, self.default_retention_days
                )
            ]
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            by_type = Counter(
                i.incident_type.value for i in self.incidents.values()
            )
            by_status = Counter(
                i.status.value for i in self.incidents.values()
            )
...
class PatternRecognizer:
    ...
    def __init__(self, min_occurrences: int = 3):
        ...
        self._lock = threading.Lock()
        logger.info(
            f"PatternRecognizer initialized (min_occurrences: {min_occurrences})"
        )
    ...
    def analyze_incident(self, incident: Incident) -> Optional[Pattern]:
        ...
        with self._lock:
            if signature in self.patterns:
                ...
                logger.debug(
                    f"Updated pattern {pattern.pattern_id} (count: {pattern.occurrence_count})"
                )
                return pattern
            else:
                # Create new pattern
    ...
    def get_patterns(
        ...
        patterns = [
            p for p in patterns if p.occurrence_count >= self.min_occurrences
        ]
        ...
        if incident_type is not None:
            patterns = [
                p for p in patterns if p.incident_type == incident_type
            ]
...
class IncidentMemorySystem:
    ...
    def record_incident(
        ...
        if pattern and pattern.occurrence_count >= 3:
            ...
            incident.related_incidents = [
                ...
                if i.get_signature() == pattern.signature
                and i.incident_id != incident.incident_id
            ][
                :10
            ]  # Limit to 10 related incidents
            self.incident_store.store_incident(
                incident
            )  # Update with new status

            logger.warning(
                f"Recurring pattern detected: {pattern.pattern_id} "
    ...
    def maintenance(self) -> Dict[str, int]:
        ...
        logger.info(
            f"Maintenance: cleared {expired_incidents} incidents, {old_patterns} patterns"
        )
...
def create_incident_from_alert(
    alert_data: Dict[str, Any], source: str
) -> Incident:
...
def create_incident_from_failure(
    failure_data: Dict[str, Any], source: str
) -> Incident:
    """Create an incident from a failure."""
    return Incident(
        incident_id=failure_data.get(
