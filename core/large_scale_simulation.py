...
class LargeScaleSimulation:
    ...
    def _establish_connections(self):
        ...
        for node_id, node in self.nodes.items():
            ...
            connections = random.sample(
                nearby,
                min(len(nearby), max_connections)
            )
    ...
    def _simulate_chaos(self, delta_time: float):
        ...
        for node in self.nodes.values():
            if node.state != NodeState.FAILED and random.random() < failure_probability:
    ...
    def get_current_metrics(self) -> SimulationMetrics:
        ...
        avg_connections = statistics.mean(connection_counts) if connection_counts else 0.0
