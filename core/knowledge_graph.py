...
class SolutionStrategy:
    ...
    def record_outcome(
        self, outcome: OutcomeType, resolution_time: float
    ) -> None:
        ...
        if outcome == OutcomeType.SUCCESS:
            ...
            self.average_resolution_time = (
                self.average_resolution_time * (total_count - 1)
                + resolution_time
            ) / total_count
...
class KnowledgeEdge:
    ...
    def update_weight(
        self, outcome: OutcomeType, learning_rate: float = 0.1
    ) -> None:
        """
...
class KnowledgeGraph:
    ...
    def __init__(
        self, learning_rate: float = 0.1, max_experiences: int = 10000
    ):
        """
    ...
    def recognize_pattern(
        self, description: str, features: Dict[str, Any], severity: float = 0.5
    ) -> str:
        """
        ...
        for pattern in self.problem_patterns.values():
            if pattern.signature == signature:
                ...
                logger.debug(
                    f"Recognized existing pattern: {pattern.pattern_id}"
                )
    ...
    def register_strategy(
        ...
        """
        strategy_id = (
            f"strategy_{hashlib.sha256(name.encode()).hexdigest()[:16]}"
        )
    ...
    def record_incident(
        ...
        if edge_key not in self.knowledge_edges:
            self.knowledge_edges[edge_key] = KnowledgeEdge(
                source_pattern_id=pattern_id, target_strategy_id=strategy_id
            )
    ...
    def _perform_root_cause_analysis(
        self, pattern_id: str, strategy_id: str
    ) -> str:
        """
        ...
        if pattern and strategy:
            ...
            if strategy.failure_count > strategy.success_count:
                suspected_causes.append(
                    "Strategy has historically high failure rate"
                )
                recommendations.append(
                    "Consider alternative strategies with better track record"
                )

            # Check if problem severity is too high for this strategy
            if pattern.severity > 0.8 and strategy.resource_cost < 2.0:
                suspected_causes.append(
                    "Problem severity may exceed strategy capability"
                )
                recommendations.append(
                    "Apply more resource-intensive strategies for severe problems"
                )

            # Check if pattern is novel
            if pattern.occurrence_count <= 2:
                suspected_causes.append(
                    "Novel problem pattern with limited data"
                )
                recommendations.append(
                    "Gather more data and try diverse strategies"
                )
        ...
        logger.warning(
            f"Root cause analysis: {len(suspected_causes)} causes identified"
        )

        return analysis_id

    def get_best_strategy(
        self, pattern_id: str, top_k: int = 3
    ...
    def get_experiences_for_pattern(
        self, pattern_id: str
    ) -> List[IncidentExperience]:
        """Get all RL experiences related to a pattern."""
        return [
            exp for exp in self.experiences if exp.pattern_id == pattern_id
        ]
    ...
    def export_graph(self, filepath: str) -> None:
        ...
        data = {
            "patterns": {
                k: asdict(v) for k, v in self.problem_patterns.items()
            },
            "strategies": {
                k: asdict(v) for k, v in self.solution_strategies.items()
            },
            "edges": {
