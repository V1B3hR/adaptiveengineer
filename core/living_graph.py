...
class GraphNode:
    ...
    def update_stress(self, delta: float):
        ...
        self.attributes.stress_level = max(0.0, min(1.0, 
    ...
    def can_host_agent(self) -> bool:
        ...
        return (
            self.attributes.current_agents < self.attributes.agent_capacity and
            self.attributes.health_status == HealthStatus.OK and
            self.attributes.stress_level < 0.8
        )
    ...
    def provide_energy(self) -> float:
        ...
        stress_penalty = self.attributes.stress_level * 0.3  # But more stress reduces it
...
class LivingGraph:
    ...
    def __init__(self, seed: Optional[int] = None):
        ...
        self.adjacency: Dict[str, Set[str]] = defaultdict(set)  # node_id -> set of connected node_ids
        self.reverse_adjacency: Dict[str, Set[str]] = defaultdict(set)  # For incoming edges
    ...
    def add_node(self, node_type: NodeType, name: str, 
                 ...
                 attributes: Optional[NodeAttributes] = None) -> str:
        """
    ...
    def add_edge(self, source_id: str, target_id: str, edge_type: EdgeType,
                 attributes: Optional[EdgeAttributes] = None) -> str:
        """
        ...
        logger.debug(f"Added edge from {source_id} to {target_id} ({edge_type})")
    ...
    def _update_node(self, node: GraphNode, delta_time: float):
        ...
        node.attributes.memory_usage = max(0.0, min(1.0, node.attributes.memory_usage))
        ...
        if node.attributes.health_status == HealthStatus.DEGRADED and np.random.random() < 0.005:
            ...
            self._record_event("node_failure", {"node_id": node.node_id, "node_name": node.name})
    ...
    def _update_edge(self, edge: GraphEdge, delta_time: float):
        ...
        edge.attributes.bandwidth_utilization = max(0.0, min(1.0, edge.attributes.bandwidth_utilization))
        
        # High utilization can cause packet drops
        if edge.attributes.bandwidth_utilization > 0.9:
            edge.attributes.packet_drop_rate = min(0.5, edge.attributes.packet_drop_rate + 0.01)
        else:
            edge.attributes.packet_drop_rate = max(0.0, edge.attributes.packet_drop_rate - 0.01)
            ...
            if node.attributes.health_status == HealthStatus.FAILED:
                ...
                for dependent_id in self.get_neighbors(node_id):
                    if dependent_id in self.nodes:
                        ...
                        dependent.attributes.stress_level = min(1.0, 
                            dependent.attributes.stress_level + 0.2)
                        logger.debug(f"Failure of {node.name} increased stress on {dependent.name}")
    
    def simulate_ddos_attack(self, target_node_id: str, intensity: float = 0.8):
        """
        ...
        for source_id in self.get_dependencies(target_node_id):
            for edge_id, edge in self.edges.items():
                if edge.source_id == source_id and edge.target_id == target_node_id:
                    edge.attributes.bandwidth_utilization = min(1.0, 
                        edge.attributes.bandwidth_utilization + intensity)
                    edge.attributes.packet_drop_rate = min(0.9, 
                        edge.attributes.packet_drop_rate + intensity * 0.5)
        
        # Increase node stress and CPU
        node.attributes.stress_level = min(1.0, node.attributes.stress_level + intensity)
        node.attributes.cpu_load = min(1.0, node.attributes.cpu_load + intensity * 0.7)
        node.attributes.active_threat_score = min(1.0, node.attributes.active_threat_score + intensity)
        
        self._record_event("ddos_attack", {
            "target_node_id": target_node_id,
            "target_name": node.name,
            "intensity": intensity
        })
        
        logger.warning(f"DDoS attack on {node.name} with intensity {intensity}")
    
    def simulate_resource_exhaustion(self, node_id: str, resource: str = "cpu"):
        """
        ...
        node.attributes.stress_level = min(1.0, node.attributes.stress_level + 0.5)
        
        self._record_event("resource_exhaustion", {
            "node_id": node_id,
            "node_name": node.name,
            "resource": resource
        })
    ...
    def add_vulnerability(self, node_id: str, severity: float = 0.5):
        ...
        node.attributes.active_threat_score = min(1.0, 
            node.attributes.active_threat_score + severity * 0.3)
        
        self._record_event("vulnerability_added", {
            "node_id": node_id,
            "node_name": node.name,
            "severity": severity
        })
        
        logger.info(f"Added vulnerability (severity {severity}) to {node.name}")
    ...
    def get_edge_state(self, source_id: str, target_id: str) -> Optional[Dict[str, Any]]:
    ...
    def query_nodes(self, node_type: Optional[NodeType] = None,
                   ...
                   max_stress: Optional[float] = None) -> List[str]:
        """
        ...
        for node_id, node in self.nodes.items():
            if node_type and node.node_type != node_type:
                continue
            if health_status and node.attributes.health_status != health_status:
                continue
            if max_stress is not None and node.attributes.stress_level > max_stress:
                continue
