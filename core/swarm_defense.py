...
class SwarmDefenseSystem:
    ...
    def __init__(
        ...
        logger.info(f"Swarm defense system initialized with {agent_count} agents "
    ...
    def _initiate_swarm_response(self, threat_zone: ThreatZone):
        ...
        nearby_agents = self._find_nearby_agents(
            threat_zone.center,
            radius=self.coordination_range
        )
        
        # Recruit additional agents based on threat level
        required_agents = self._calculate_required_agents(threat_zone.threat_level)
        ...
        if len(nearby_agents) < required_agents:
            ...
            nearby_agents = self._recruit_agents(
                threat_zone.center,
                required_agents - len(nearby_agents)
            )
        ...
        success = self._execute_swarm_strategy(threat_zone, strategy, nearby_agents)
        ...
        if success:
            ...
            logger.info(f"Swarm successfully neutralized threat {threat_zone.zone_id} "
                       f"with {len(nearby_agents)} agents in {duration:.2f}s")
        else:
            logger.warning(f"Swarm failed to neutralize threat {threat_zone.zone_id}")
    ...
    def _recruit_agents(
        ...
        available.sort(
            key=lambda aid: self._calculate_distance(self.agents[aid].position, location)
        )
    ...
    def _execute_swarm_strategy(
        ...
        effectiveness = min(1.0, len(agent_ids) / self._calculate_required_agents(threat_zone.threat_level))
    ...
    def _store_threat_pattern(self, threat_zone: ThreatZone, response: SwarmResponse):
        """
        ...
        memory_agents = [
            ...
            if a.role == SwarmRole.MEMORY_CELL
        ]
    ...
    def recall_threat_pattern(
        ...
        """
        matching = [
            p for p in self.threat_patterns 
        ...
        for agent in self.agents.values():
            ...
            # Other agents follow pheromone trails
                ...
                target = self._find_strongest_pheromone(
                    agent.position,
                    [PheromoneType.THREAT, PheromoneType.HELP]
                )
    ...
    def get_swarm_metrics(self) -> Dict[str, Any]:
        ...
        active_threats = sum(
            1 for zone in self.threat_zones.values() 
            if not zone.neutralized
        )
        ...
        neutralization_rate = (
            self.collective_threats_neutralized / self.collective_threats_detected
            if self.collective_threats_detected > 0 else 0.0
        )
    ...
    def visualize_swarm_state(self) -> str:
        ...
        lines = []
        lines.append(f"\n{'='*60}")
        lines.append(f"SWARM STATE (Network: {self.network_size}x{self.network_size})")
        lines.append(f"{'='*60}")
        ...
        for zone in active:
            lines.append(f"  {zone.zone_id}: {zone.threat_level.value} "
        ...
        for p in self.pheromones:
            pheromone_counts[p.pheromone_type.value] = pheromone_counts.get(p.pheromone_type.value, 0) + 1
