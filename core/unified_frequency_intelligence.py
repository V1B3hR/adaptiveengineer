...
from core.behavioral_frequency import (
    BehavioralFrequencyAnalyzer,
    BehaviorThreat,
)
from core.keystroke_frequency import (
    KeystrokeFrequencyAnalyzer,
...
class ThreatCorrelationType(str, Enum):
    ...
    PHYSICAL_CYBER_ATTACK = (
        "physical_cyber_attack"  # RF jamming + network disruption
    )
    APT_CAMPAIGN = "apt_campaign"  # Ultrasonic + C2 beacon
    INSIDER_THREAT = "insider_threat"  # Keystroke anomaly + data exfiltration
    COORDINATED_ATTACK = (
        "coordinated_attack"  # Multiple domains simultaneously
    )
    RECONNAISSANCE = (
        "reconnaissance"  # Port scan + acoustic/vibration monitoring
    )
    NONE = "none"
...
class UnifiedFrequencyIntelligence:
    ...
    def __init__(
        ...
        self.acoustic_analyzer = (
            AcousticFrequencyAnalyzer() if enable_acoustic else None
        )
        self.vibration_analyzer = (
            VibrationFrequencyAnalyzer() if enable_vibration else None
        )
        self.network_analyzer = (
            NetworkFrequencyAnalyzer() if enable_network else None
        )
        self.behavior_analyzer = (
            BehavioralFrequencyAnalyzer() if enable_behavioral else None
        )
        self.keystroke_analyzer = (
            KeystrokeFrequencyAnalyzer() if enable_keystroke else None
        )
    ...
    def analyze_all_frequencies(
        self, battery_level: float = 1.0, duration_seconds: float = 1.0
        ...
        adaptive_duration = (
            duration_seconds if battery_level > 0.3 else duration_seconds * 0.5
        )
        ...
        if self.rf_analyzer:
            try:
                signals = self.rf_analyzer.scan_spectrum(
                    adaptive_duration, battery_level=battery_level
                )
        ...
        if self.acoustic_analyzer:
            try:
                events = self.acoustic_analyzer.analyze_audio_sample(
        ...
        if self.vibration_analyzer:
            try:
                events = self.vibration_analyzer.analyze_vibration(
                    duration_seconds=adaptive_duration
                )
        ...
        if self.network_analyzer:
            try:
                ...
                network_threats = sum(
                    len(threats) for threats in results.values()
                )
            except Exception as e:
        ...
        if (
            self.baseline_environment is None
            and len(self.environment_history) >= 10
        ):
    ...
    def _correlate_threats(self) -> List[ThreatCorrelationType]:
        ...
        rf_threats = (
            self.rf_analyzer.detected_threats[-10:] if self.rf_analyzer else []
        )
        acoustic_events = (
            self.acoustic_analyzer.detected_events[-10:]
            if self.acoustic_analyzer
            else []
        )
        network_threats = (
            self.network_analyzer.detected_threats[-10:]
            if self.network_analyzer
            else []
        )
        behavior_threats = (
            self.behavior_analyzer.detected_threats[-10:]
            if self.behavior_analyzer
            else []
        )
        auth_results = (
            self.keystroke_analyzer.authentication_results[-10:]
            if self.keystroke_analyzer
            else []
        )
        ...
        has_network_disruption = any(
            t.threat_level.value in ["high", "critical"]
            for t in network_threats
        )
        ...
        has_ultrasonic = any(
            e.signature.value == "ultrasonic_communication"
            for e in acoustic_events
        )
        has_c2_beacon = any(
            t.threat_type.value == "c2_beacon" for t in network_threats
        )
        ...
        has_auth_failure = any(
            r.status.value in ["rejected", "suspicious"] for r in auth_results
        )
        has_malicious_behavior = any(
            t.severity.value in ["malicious", "critical"]
            for t in behavior_threats
        )
        ...
        has_port_scan = any(
            t.threat_type.value == "port_scan" for t in network_threats
        )
    ...
    def _create_correlation(
        ...
        logger.warning(
            f"Correlated threat: {correlation_type.value} - {description}"
        )

        return correlation

    def _generate_countermeasures(
        self, correlation_type: ThreatCorrelationType, domains: List[str]
    ) -> List[str]:
        """
    ...
    def _establish_baseline(self):
        ...
        avg_env = FrequencyEnvironment(
            ...
            vibration_events=int(
                np.mean([e.vibration_events for e in recent])
            ),
            network_threats=int(np.mean([e.network_threats for e in recent])),
            behavior_threats=int(
                np.mean([e.behavior_threats for e in recent])
            ),
            authentication_status="authenticated",
            overall_threat_score=float(
                np.mean([e.overall_threat_score for e in recent])
            ),
            active_correlations=[],
        )
    ...
    def get_threat_report(self) -> Dict[str, Any]:
        ...
        """
        current_env = (
            self.environment_history[-1] if self.environment_history else None
        )
        ...
        report = {
            ...
            "current_environment": {
                "overall_threat_score": (
                    current_env.overall_threat_score if current_env else 0.0
                ),
                "rf_signals": current_env.rf_signals if current_env else 0,
                "rf_threats": current_env.rf_threats if current_env else 0,
                "acoustic_events": (
                    current_env.acoustic_events if current_env else 0
                ),
                "vibration_events": (
                    current_env.vibration_events if current_env else 0
                ),
                "network_threats": (
                    current_env.network_threats if current_env else 0
                ),
                "behavior_threats": (
                    current_env.behavior_threats if current_env else 0
                ),
                "authentication_status": (
                    current_env.authentication_status
                    if current_env
                    else "unknown"
                ),
            },
            "active_correlations": (
                [c.value for c in current_env.active_correlations]
                if current_env
                else []
            ),
    ...
    def _get_recommendations(self) -> List[str]:
        ...
        recent_critical = [
            c
            for c in self.correlated_threats
        ...
        if recent_critical:
            ...
            for threat in recent_critical[-3:]:  # Last 3 critical threats
                recommendations.extend(
                    threat.countermeasures[:2]
                )  # Top 2 countermeasures
        else:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        if self.acoustic_analyzer:
            stats["acoustic_analyzer"] = (
                self.acoustic_analyzer.get_statistics()
            )
        if self.vibration_analyzer:
            stats["vibration_analyzer"] = (
                self.vibration_analyzer.get_statistics()
            )
        if self.network_analyzer:
            stats["network_analyzer"] = self.network_analyzer.get_statistics()
        if self.behavior_analyzer:
            stats["behavior_analyzer"] = (
                self.behavior_analyzer.get_statistics()
            )
        if self.keystroke_analyzer:
            stats["keystroke_analyzer"] = (
                self.keystroke_analyzer.get_statistics()
            )
