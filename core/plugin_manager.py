...
class PluginManager:
    ...
    def register_plugin(self, plugin: PluginBase) -> bool:
        ...
        logger.info(
            f"Registered plugin: {plugin.plugin_id} (type: {plugin_type})"
        )
        return True
    ...
    def initialize_all(self, node: Any) -> None:
        ...
        for plugin in self.plugins.values():
            try:
                ...
                logger.error(
                    f"Failed to initialize plugin {plugin.plugin_id}: {e}"
                )
        ...
        for plugin in self.plugins.values():
            if plugin.is_enabled():
                try:
                    ...
                    logger.error(
                        f"Error updating plugin {plugin.plugin_id}: {e}"
                    )
        ...
        for plugin in self.plugins.values():
            if plugin.is_enabled():
                try:
                    ...
                    logger.error(
                        f"Error processing signal in plugin {plugin.plugin_id}: {e}"
                    )
        return responses

    def execute_action(
        self, plugin_id: str, action_type: str, params: Dict[str, Any]
    ) -> bool:
        """
