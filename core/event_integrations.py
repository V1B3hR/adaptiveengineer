...
class SIEMStream(EventStream):
    ...
    def close(self) -> None:
        """Close SIEM stream."""
        logger.info(
            f"SIEMStream closed ({self.event_count} events, {self.security_event_count} security events)"
        )
...
class AIOpsStream(EventStream):
    ...
    def _format_for_aiops(self, event: Event) -> Dict[str, Any]:
        ...
        return {
            ...
            "metrics": event.data,
            "tags": event.tags,
            "metadata": {"platform": "adaptive_engineer", "version": "2.0"},
        }

    def close(self) -> None:
        """Close AIOps stream."""
        logger.info(
            f"AIOpsStream closed ({self.event_count} events, {self.anomaly_count} anomalies)"
        )
...
class EventIntegrationManager:
    ...
    def publish_from_message(self, message: Message) -> int:
        ...
        event = Event(
            ...
            severity=severity,
            source=(
                f"node_{message.source_node_id}"
        ...
        with self._lock:
            for name, stream in self.streams.items():
                try:
                    ...
                    logger.error(
                        f"Error closing stream {name}: {e}", exc_info=True
                    )
            self.streams.clear()
            self.filters.clear()

        logger.info(
            f"All streams closed ({self.total_events} total events processed)"
        )


# Convenience functions
def create_webhook_integration(
    url: str, event_filter: Optional[EventFilter] = None
) -> tuple:
    """
...
def create_siem_integration(
    ...
    severity_threshold: EventSeverity = EventSeverity.WARNING,
) -> tuple:
    """
...
def create_aiops_integration(
    aiops_config: Dict[str, Any], tags: Optional[List[str]] = None
) -> tuple:
    """
