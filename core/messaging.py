...
class MessageBus(ABC):
    ...
    def register_handler(self, handler: MessageHandler) -> None:
        ...
        with self._lock:
            ...
            logger.debug(
                f"Registered handler for message types: {handler.message_types}"
            )
    ...
    def _is_duplicate(self, message: Message) -> bool:
        ...
        with self._lock:
            ...
            if content_hash in self.processed_messages:
                logger.debug(
                    f"Duplicate message detected: {message.message_id}"
                )
                return True
    ...
    def _dispatch_message(self, message: Message) -> None:
        ...
        if message.is_expired():
            logger.warning(
                f"Message {message.message_id} expired (TTL: {message.ttl}s)"
            )
            return
    ...
    def replay_messages(
        ...
        if message_types is not None:
            ...
            messages = [
                m for m in messages if m["message_type"] in type_values
            ]
...
class InMemoryMessageBus(MessageBus):
    ...
    def send(self, message: Message) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Message {message.message_id} queued (type: {message.message_type.value})"
            )
            return True
        except Exception as e:
...
def create_message_bus(
    backend: str = "memory", node_id: Optional[int] = None, **kwargs
) -> MessageBus:
    """
    ...
    if backend == "memory":
        ...
        logger.warning(
            f"Backend '{backend}' not yet implemented, using in-memory bus"
        )
        return InMemoryMessageBus(node_id=node_id)
