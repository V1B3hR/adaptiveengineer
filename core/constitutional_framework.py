...
class ActionEvaluation:
    ...
    is_compliant: bool
    violations: List[Tuple[ConstitutionalLaw, str]] = field(default_factory=list)
...
class ConstitutionalFramework:
    ...
    def evaluate_action(self,
                       ...
                       agent_id: Optional[str] = None) -> ActionEvaluation:
        """
        ...
        if not integrity_check[0]:
            violations.append((ConstitutionalLaw.SYSTEM_INTEGRITY, integrity_check[1]))
        ...
        if not continuity_check[0]:
            violations.append((ConstitutionalLaw.OPERATIONAL_CONTINUITY, continuity_check[1]))
            evaluation.is_compliant = False
        ...
        if not evolution_check[0]:
            violations.append((ConstitutionalLaw.EFFICIENT_EVOLUTION, evolution_check[1]))
            evaluation.is_compliant = False
        ...
        if evaluation.is_compliant:
            ...
            self._record_violation(action_id, action_description, violations, 
                                  agent_id, action_context)
        
        if violations:
            logger.warning(f"Action {action_id} violates constitutional framework:")
            for law, reason in violations:
                logger.warning(f"  - {law.name}: {reason}")
        else:
            logger.debug(f"Action {action_id} compliant with constitutional framework "
                        f"(score={evaluation.compliance_score:.2f})")
        
        return evaluation
    
    def _check_system_integrity(self, 
                                context: Dict[str, Any]) -> Tuple[bool, str, Optional[str]]:
        """
        ...
        if context.get('affects_critical_components'):
            ...
            critical_affected = [c for c in affected if c in self.system_components]
            
            if critical_affected and not context.get('has_redundancy'):
                return (False, f"Would affect critical components without redundancy: {critical_affected}", None)
            
            if critical_affected:
                warning = f"Action affects critical components: {critical_affected}"
                return (True, "", warning)
        
        # Check for destructive operations without backup
        if context.get('is_destructive') and not context.get('has_backup'):
            return (False, "Destructive operation without backup violates system integrity", None)
    ...
    def _check_operational_continuity(self,
                                     context: Dict[str, Any]) -> Tuple[bool, str, Optional[str]]:
        """
        ...
        if context.get('affects_services'):
            ...
            critical_affected = [s for s in affected if s in self.critical_services]
            
            if critical_affected:
                # Check if action would cause downtime
                if context.get('causes_downtime'):
                    if not context.get('scheduled_maintenance'):
                        return (False, f"Would cause unscheduled downtime for critical services: {critical_affected}", None)
                    else:
                ...
                if context.get('degrades_performance'):
                    ...
                    if severity > 0.5:
                        return (False, f"Would significantly degrade critical services: {critical_affected}", None)
                    elif severity > 0.3:
        ...
        if context.get('low_priority_action') and context.get('critical_services_degraded'):
            return (False, "Cannot perform low-priority action while critical services are degraded", None)
        
        return (True, "", None)
    
    def _check_efficient_evolution(self,
                                   context: Dict[str, Any]) -> Tuple[bool, str, Optional[str]]:
        """
        ...
        if context.get('prevents_learning'):
            return (False, "Action would prevent system learning and improvement", None)
        
        # Check if action is wasteful
        if context.get('resource_efficiency'):
            efficiency = context.get('resource_efficiency', 1.0)
            if efficiency < 0.3:
                return (False, "Action is extremely resource-inefficient", None)
            elif efficiency < 0.5:
        ...
        if context.get('degrades_capability'):
            ...
            if degradation > 0.5:
                return (False, "Action significantly degrades system capabilities", None)
        ...
        if context.get('blocks_evolution'):
            return (False, "Action would block evolutionary improvements", None)
    ...
    def _calculate_compliance_score(self,
        ...
        if context.get('affects_critical_components') or context.get('affects_services'):
            score -= 0.1
    ...
    def _record_violation(self,
        ...
        for law, reason in violations:
            ...
            logger.warning(f"Constitutional violation recorded: {violation_id}")
    ...
    def _determine_violation_severity(self,
        ...
        if context.get('affects_critical_components') or context.get('affects_services'):
            critical_affected_services = [s for s in context.get('affected_services', []) 
                                         if s in self.critical_services]
            critical_affected_components = [c for c in context.get('affected_components', [])
        ...
        if context.get('causes_downtime') or context.get('compromises_system_integrity'):
    ...
    def get_violations_by_law(self, law: ConstitutionalLaw) -> List[ConstitutionalViolation]:
        """Get all violations of a specific law"""
        return [v for v in self.violations.values() if v.law_violated == law]
    
    def get_recent_violations(self, limit: int = 10) -> List[ConstitutionalViolation]:
    ...
    def get_statistics(self) -> Dict:
        ...
        violation_severity_counts = {
            severity: sum(1 for v in self.violations.values() if v.severity == severity)
        ...
        return {
            ...
            'compliance_rate': self.total_compliant_actions / max(1, self.total_actions_evaluated),
            'violations_by_law': {
                law.name: count for law, count in self.violation_count_by_law.items()
            },
            'violations_by_severity': {
                severity.value: count for severity, count in violation_severity_counts.items()
            },
