...
class CollectiveCognitionEngine:
    ...
    def should_analyze(self) -> bool:
        ...
        if (
            self.knowledge_graph.total_incidents
            < self.min_incidents_for_analysis
        ):
            return False
    ...
    def perform_meta_learning(self) -> List[CognitiveInsight]:
        ...
        logger.info(
            f"Meta-learning complete: {len(new_insights)} new insights generated"
        )
    ...
    def _cluster_problem_patterns(self) -> List[CognitiveInsight]:
        ...
        for pattern in patterns:
            ...
            feature_keys = sorted(pattern.features.keys())[
                :3
            ]  # Top 3 features
        ...
        for cluster_key, pattern_ids in clusters.items():
            if len(pattern_ids) >= 3:  # At least 3 similar patterns
                ...
                logger.debug(
                    f"Discovered pattern cluster: {cluster_key} ({len(pattern_ids)} patterns)"
                )
        ...
        for strategy_id, pattern_ids in strategy_failures.items():
            if len(pattern_ids) >= 2:
                ...
                insight = CognitiveInsight(
                    ...
                    confidence=0.85,
                    supporting_evidence=[
                        f"pattern_{pid}" for pid in pattern_ids
                    ],
    ...
    def _identify_success_patterns(self) -> List[CognitiveInsight]:
        ...
        pattern_solutions: Dict[str, List[Tuple[str, float]]] = defaultdict(
            list
        )
    ...
    def _synthesize_hybrid_strategies(self) -> List[CognitiveInsight]:
        ...
        successful_strategies = [
            s
            for s in self.knowledge_graph.solution_strategies.values()
    ...
    def _are_complementary(self, strategy_a: Any, strategy_b: Any) -> bool:
        ...
        overlap = len(actions_a & actions_b) / min(
            len(actions_a), len(actions_b)
        )
    ...
    def _create_hybrid_strategy(
        self, strategy_a: Any, strategy_b: Any
    ) -> HybridStrategy:
        """
        ...
        est_effectiveness = (
            strategy_a.success_rate + strategy_b.success_rate
        ) / 2

        # Combined resource cost
        resource_cost = (
            strategy_a.resource_cost + strategy_b.resource_cost
        ) * 0.7  # Some efficiency

        # Novelty based on how different the parents are
        novelty = 1.0 - (
            len(set(strategy_a.actions) & set(strategy_b.actions))
    ...
    def _seed_into_evolution(self, hybrid: HybridStrategy) -> None:
        ...
        logger.debug(
            f"Seeded hybrid strategy {hybrid.name} into evolution engine"
        )
    ...
    def _detect_novel_threats(self) -> List[CognitiveInsight]:
        ...
        novel_patterns = [
            p
            for p in self.knowledge_graph.problem_patterns.values()
        ...
        for pattern in novel_patterns:
            ...
            best_strategies = self.knowledge_graph.get_best_strategy(
                pattern.pattern_id, top_k=1
            )

            if (
                not best_strategies or best_strategies[0][1] < 0.3
            ):  # No good solution
                insight = CognitiveInsight(
    ...
    def _identify_optimizations(self) -> List[CognitiveInsight]:
        ...
        for pattern_id in self.knowledge_graph.problem_patterns.keys():
            strategies = self.knowledge_graph.get_best_strategy(
                pattern_id, top_k=5
            )
            ...
            if len(strategies) >= 2:
                ...
                for strategy_id, weight in strategies:
                    if weight > 0.5:  # Only consider successful strategies
                        strategy = (
                            self.knowledge_graph.solution_strategies.get(
                                strategy_id
                            )
                        )
                        if strategy:
                            # Cost-effectiveness: weight / resource_cost
                            cost_effectiveness = weight / max(
                                0.1, strategy.resource_cost
                            )
                            strategy_costs.append(
                                (strategy_id, cost_effectiveness, strategy)
                            )
    ...
    def get_top_insights(
        self, limit: int = 10, min_priority: float = 0.5
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "high_priority_insights": len(
                [i for i in self.insights if i.priority > 0.7]
            ),
        }
