...
class Bid:
    ...
    def calculate_score(
        ...
        cost_factor = (
            max(0.0, 1.0 - (self.bid_amount / task_reward))
            if task_reward > 0
            else 0.0
        )
...
class ContractNetProtocol:
    ...
    def __init__(
        ...
        # Agent tracking
        self.agent_active_tasks: Dict[str, Set[str]] = (
            {}
        )  # agent_id -> set of task_ids
        ...
        logger.info(
            f"Contract net protocol initialized (bidding_time={default_bidding_time}s)"
        )
    ...
    def propose_task(
        ...
        logger.info(
            f"Task proposed: {task_id} - {description} "
    ...
    def submit_bid(
        ...
        if not task.requirements.is_qualified(
            ...
            logger.warning(
                f"Agent {bidder_id} does not meet requirements for task {task_id}"
            )
            return None
        ...
        logger.info(
            f"Bid submitted: {bidder_id} on {task_id} "
    ...
    def award_contract(
        ...
        if task.status != TaskStatus.ANNOUNCED:
            logger.warning(
                f"Cannot award task {task_id} with status {task.status}"
            )
            return None
    ...
    def start_task(self, task_id: str, agent_id: str) -> bool:
        ...
        if task.status != TaskStatus.AWARDED:
            logger.warning(
                f"Task {task_id} has status {task.status}, cannot start"
            )
            return False
    ...
    def report_progress(
        ...
        logger.info(
            f"Progress reported: {task_id} by {agent_id} - {progress:.1%}"
    ...
    def complete_task(
        self, task_id: str, agent_id: str, result: Optional[Dict] = None
    ) -> bool:
        """
        ...
        completion_time = (
            task.completed_at - task.awarded_at if task.awarded_at else 0
        )
        logger.info(
            f"Task completed: {task_id} by {agent_id} "
    ...
    def request_help(
        self, task_id: str, agent_id: str, help_needed: str
    ) -> bool:
        """
    ...
    def get_available_tasks(self, agent_id: str) -> List[Task]:
        ...
        return [task for task in self.tasks.values() if task.is_bidding_open()]
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            "completion_rate": self.total_tasks_completed
            / max(1, self.total_tasks_announced),
            "avg_bids_per_task": self.total_bids_received
            / max(1, self.total_tasks_announced),
        }
