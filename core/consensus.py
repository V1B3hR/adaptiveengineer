...
class ConsensusProposal:
    ...
    quorum_size: int = 0
    required_approval_ratio: float = (
        0.67  # Byzantine fault tolerance threshold
    )
...
class ByzantineDetector:
    ...
    def __init__(self, suspicion_threshold: float = 0.3):
        ...
        self.node_behavior: Dict[int, Dict[str, Any]] = defaultdict(
            lambda: {
                "total_votes": 0,
    ...
    def record_vote(
        self, node_id: int, vote: Vote, was_minority: bool
    ) -> None:
    ...
    def detect_suspicious_patterns(
        self, votes: List[Vote], consensus_result: VoteType
    ) -> Set[int]:
        """
        ...
        for vote in votes:
            ...
            if behavior["total_votes"] > 5:
                minority_ratio = (
                    behavior["minority_votes"] / behavior["total_votes"]
                )
                ...
                if behavior["total_votes"] > 3:
                    abstain_ratio = (
                        sum(
                            ...
                            and v.vote_type == VoteType.ABSTAIN
                        )
                        / behavior["total_votes"]
                    )
    ...
    def get_suspicion_score(self, node_id: int) -> float:
        ...
        minority_ratio = behavior["minority_votes"] / max(
            1, behavior["total_votes"]
        )
...
class ConsensusEngine:
    ...
    def __init__(
        ...
        logger.info(
            f"Consensus engine initialized for node {node_id} "
    ...
    def initiate_consensus(
        ...
        logger.info(
            f"Initiated consensus {proposal_id} for {consensus_type} "
    ...
    def cast_vote(
        ...
        if proposal.consensus_reached:
            return {"accepted": False, "reason": "Proposal already finalized"}

        # Check deadline
        if proposal.deadline and time.time() > proposal.deadline:
            return {"accepted": False, "reason": "Proposal deadline passed"}
        ...
        logger.debug(
            f"Vote cast on {proposal_id} by node {voter_id}: {vote_type}"
        )
    ...
    def _check_consensus(self, proposal: ConsensusProposal) -> None:
        ...
        if proposal.consensus_reached:
            ...
            for vote in proposal.votes:
                ...
                self.byzantine_detector.record_vote(
                    vote.voter_id, vote, was_minority
                )

            # Detect suspicious patterns
            suspicious = self.byzantine_detector.detect_suspicious_patterns(
                proposal.votes, proposal.consensus_result
            )

            logger.info(
                f"Consensus reached on {proposal.proposal_id}: "
    ...
    def get_proposal_status(
        ...
        return {
            ...
            "time_remaining": (
                max(0, proposal.deadline - time.time())
    ...
    def get_suspicious_nodes(self) -> List[Dict[str, Any]]:
                ...
                suspicious.append(
                    {
                        ...
                        "total_votes": self.byzantine_detector.node_behavior[
                            node_id
                        ]["total_votes"],
                        "minority_votes": self.byzantine_detector.node_behavior[
                            node_id
                        ][
                            "minority_votes"
                        ],
        ...
        return sorted(
            suspicious, key=lambda x: x["suspicion_score"], reverse=True
        )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        finalized = sum(
            1 for p in self.proposals.values() if p.consensus_reached
        )
        ...
        return {
            ...
            "trusted_nodes_count": len(
                [t for t in self.node_trust_scores.values() if t > 0.7]
            ),
        }
