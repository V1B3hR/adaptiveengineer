...
from core.behavior_strategy import (
    BehaviorStrategy, BehaviorNode, FiniteStateMachine, FSMTransition,
    BehaviorStrategyFactory, AgentRole, NodeType, StateType, BehaviorStatus
)
from core.swarm_intelligence import (
    SwarmIntelligenceManager, PheromoneType, StructuredPheromone
)
from core.predictive_homeostasis import (
    PredictiveHomeostasisSystem, FailureType, StressType
)
...
def test_behavior_strategies():
    ...
    print(f"✓ Specialization: Scout={scout_spec:.3f}, Generalist={generalist_spec:.3f}")
    
    # Test behavior tree execution
    context = {
        "conditions": {
            "check_energy": lambda ctx, params: ctx.get("energy", 1.0) > params.get("threshold", 0.5)
        },
        "actions": {
    ...
    if scout.behavior_tree:
        ...
        assert status in [BehaviorStatus.SUCCESS, BehaviorStatus.FAILURE, BehaviorStatus.RUNNING]
        print(f"✓ Behavior tree executed: {status.value}")
    
    # Test FSM state transitions
    fsm_context = {
        "conditions": {
            "detect_threat": lambda ctx, params: ctx.get("threat_level", 0) > 0.7,
            "threat_cleared": lambda ctx, params: ctx.get("threat_level", 0) < 0.3,
    ...
    if scout.fsm:
        ...
        print(f"✓ FSM transition: {initial_state.value} -> {new_state.value} (changed: {changed})")
...
def test_swarm_intelligence():
    ...
    success = manager.reinforce_pheromone(pheromone1.pheromone_id, agent_id=2, amount=0.2)
    assert success
...
def test_predictive_homeostasis():
    ...
    current_metrics = {
        "latency_edge_A": 105.0,
        "cpu_node_B": 0.92
    }
    
    # Make prediction (initially low confidence, but should detect pattern)
    predictions = system.predict_failure(current_metrics, confidence_threshold=0.0)
    ...
    if predictions:
        ...
        predictions = system.predict_failure(current_metrics, confidence_threshold=0.5)
    ...
    print(f"✓ Adaptive reallocation: {len(reallocation['reallocations'])} entities affected")
    ...
    for stress in stress_states:
        print(f"  {stress.stress_type.value}: {len(stress.affected_nodes)} nodes, severity={stress.severity:.2f}")
...
def test_phase2_plugin_integration():
    ...
    phase2_plugin = Phase2EmergencePlugin(
        plugin_id="phase2_test",
        config={"initial_role": "scout"}
    )
    ...
    print(f"✓ Plugin initialized with role: {phase2_plugin.assigned_role.value}")
    ...
    success = phase2_plugin.execute_action("deposit_pheromone", {
        "type": "threat",
    ...
    assert success
    pheromones_deposited = phase2_plugin.state_variables["pheromones_deposited"].value
    assert pheromones_deposited > 0
    print("✓ Pheromone deposition action works")
    
    # Test supply chain establishment action
    success = phase2_plugin.execute_action("establish_supply_chain", {
    ...
    success = phase2_plugin.execute_action("predict_failure", {
...
def test_emergent_behaviors():
    ...
    for i, role in enumerate([AgentRole.SCOUT, AgentRole.HARVESTER, AgentRole.GUARDIAN]):
        agent = AliveLoopNode(
            position=(0, 0),
            velocity=(0, 0),
            initial_energy=10.0,
    ...
    print(f"✓ Created population of {len(agents_data)} agents with diverse roles")
    ...
    print(f"✓ Collective pheromone field: {stats['total_pheromones']} pheromones")
    ...
    print(f"✓ Coordinated action with {coordination.total_agents()} agents (diverse: {coordination.is_diverse()})")
