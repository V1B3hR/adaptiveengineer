...
def test_attacker_learning():
    ...
    initial_mutation_rate = attacker.attack_strategies[AttackType.ENERGY_DRAIN].mutation_rate
    
    # Learn from detection
    attacker.learn_from_detection(AttackType.ENERGY_DRAIN, detected=True)
    
    # Mutation rate should increase
    new_mutation_rate = attacker.attack_strategies[AttackType.ENERGY_DRAIN].mutation_rate
...
def test_attacker_evolution():
    ...
    evolved_count = sum(
        1 for attack_type, strategy in attacker.attack_strategies.items()
