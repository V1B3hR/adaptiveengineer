...
from plugins.acoustic_frequency import (
    AcousticFrequencyAnalyzer,
    AcousticSignature,
)
...
def test_rf_spectrum_analyzer():
    ...
    assert (
        len(analyzer.authorized_transmitters) > 0
    ), "Should have authorized transmitters"

    # Test analysis
    results = analyzer.analyze_spectrum(
        duration_seconds=1.0, battery_level=1.0
    )
    assert (
        "signals_detected" in results
    ), "Results should include signals_detected"
...
def test_acoustic_frequency_analyzer():
    ...
    assert isinstance(
        events, list
    ), "analyze_audio_sample should return a list"

    # Test machinery diagnostics
    diagnostic = analyzer.diagnose_machinery(
        machine_id="test_machine", duration_seconds=2.0
    )
...
def test_vibration_frequency_analyzer():
    ...
    assert (
        "test_bearing" in analyzer.bearing_geometries
    ), "Bearing should be registered"
    ...
    events = analyzer.analyze_vibration(
        bearing_id="test_bearing", duration_seconds=1.0
    )
    ...
    health = analyzer.monitor_structural_health(
        "test_structure", duration_seconds=2.0
    )
    ...
    stats = analyzer.get_statistics()
    assert (
        "bearings_monitored" in stats
    ), "Stats should include bearings_monitored"
...
def test_network_frequency_analyzer():
    ...
    assert isinstance(
        c2_threats, list
    ), "detect_c2_beaconing should return a list"
...
def test_behavioral_frequency_analyzer():
    ...
    if len(miner_threats) > 0:
        assert any(
            t.malware_type == MalwareType.CRYPTOMINER for t in miner_threats
        ), "Should detect cryptominer"

    # Test statistics
    stats = analyzer.get_statistics()
    assert (
        "processes_tracked" in stats
    ), "Stats should include processes_tracked"
...
def test_unified_frequency_intelligence():
    ...
    environment = unified.analyze_all_frequencies(
        battery_level=1.0, duration_seconds=1.0
    )

    assert hasattr(
        environment, "overall_threat_score"
    ), "Should have threat score"
    assert (
        0 <= environment.overall_threat_score <= 1.0
    ...
    assert (
        "current_environment" in report
    ), "Report should include current_environment"
...
def test_node_integration():
    ...
    assert (
        "overall_threat_score" in results
    ), "Results should include threat score"

    # Test threat adaptation
    actions = node.adapt_to_frequency_threats()
    assert isinstance(
        actions, list
    ), "adapt_to_frequency_threats should return a list"
