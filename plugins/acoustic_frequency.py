...
class AcousticFrequencyAnalyzer:
    ...
    def __init__(
        ...
        logger.info(f"Acoustic Analyzer initialized: {sample_rate_hz/1000:.1f} kHz sample rate, "
                   f"FFT size: {fft_size}")
    ...
    def analyze_audio_sample(
        ...
        ultrasonic_events = self._detect_ultrasonic_communication(freqs, power_spectrum)
        ...
        for peak_freq, peak_power in peaks:
            ...
            for sig_name, sig_info in self.signature_database.items():
                ...
                if freq_min <= peak_freq <= freq_max:
                    ...
                    confidence = 1.0 - abs(peak_freq - sig_info["peak_freq"]) / freq_max
            ...
            if best_match:
                ...
                events.append(event)
                logger.info(f"Detected {best_match}: {peak_freq:.1f} Hz, confidence: {best_confidence:.2f}")
    ...
    def _detect_ultrasonic_communication(
        ...
        mean_audible_power = np.mean(power_spectrum[freqs < self.ultrasonic_threshold_hz])
        ...
        if mean_ultrasonic_power > mean_audible_power * 2.0:
            ...
            event = AcousticEvent(
                ...
                frequency_profile=power_spectrum,
                metadata={
                    "ultrasonic_power_ratio": float(mean_ultrasonic_power / mean_audible_power)
                }
            )
            events.append(event)
            logger.warning(f"Ultrasonic communication detected at {peak_freq:.1f} Hz")
    ...
    def diagnose_machinery(
        ...
        if len(harmonics) > 5:
            anomalies.append("Excessive harmonics detected - possible bearing wear")
            health_score -= 0.2
        
        # Check for sidebands (indicates modulation/issues)
        sidebands = self._detect_sidebands(freqs, power_spectrum, fundamental_freq)
        if sidebands:
            anomalies.append(f"Sidebands detected at {len(sidebands)} frequencies - possible misalignment")
            health_score -= 0.15
        ...
        if signal_to_noise < 10:
            anomalies.append("Low signal-to-noise ratio - sensor issue or excessive vibration")
            health_score -= 0.1
        ...
        logger.info(f"Machinery diagnostic for {machine_id}: health={health_score:.2f}, "
    ...
    def get_frequency_range_analysis(
        ...
        for range_name, (f_min, f_max) in ranges.items():
            ...
            energy_dist[range_name] = float(range_energy / total_energy if total_energy > 0 else 0)
