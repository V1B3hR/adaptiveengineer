...
class VibrationFrequencyAnalyzer:
    ...
    def __init__(
        ...
        logger.info(
            f"Vibration Analyzer initialized: {sample_rate_hz/1000:.1f} kHz sample rate, "
    ...
    def calculate_bearing_frequencies(
        ...
        bpfo = (geometry.num_balls * shaft_freq_hz / 2) * (
            1 - d_ratio * np.cos(contact_angle_rad)
        )

        # Ball Pass Frequency Inner race
        bpfi = (geometry.num_balls * shaft_freq_hz / 2) * (
            1 + d_ratio * np.cos(contact_angle_rad)
        )

        # Ball Spin Frequency
        bsf = (
            (geometry.pitch_diameter_mm / (2 * geometry.ball_diameter_mm))
            * shaft_freq_hz
        ...
        logger.debug(
            f"Bearing frequencies: BPFO={bpfo:.2f} Hz, BPFI={bpfi:.2f} Hz, "
    ...
    def _compute_fft(
        self, vibration_data: np.ndarray
        ...
        for peak_freq, peak_power in peaks:
            for defect_type, expected_freq in defect_mapping.items():
                if abs(peak_freq - expected_freq) <= tolerance_hz:
                    ...
                    confidence = 1.0 - min(
                        1.0, abs(peak_freq - expected_freq) / tolerance_hz
                    )
                    ...
                    events.append(event)
                    logger.warning(
                        f"Bearing defect detected: {defect_type.value} at {peak_freq:.2f} Hz, "
    ...
    def _analyze_general_vibration(
        ...
        for band_name, (f_min, f_max) in bands.items():
            ...
            if (
                band_power > total_power * 0.1
            ):  # If band has >10% of total energy
    ...
    def _classify_vibration_severity(
        self, amplitude_g: float
    ) -> VibrationSeverity:
        """
    ...
    def monitor_structural_health(
        ...
        peaks = self._find_spectrum_peaks(
            freqs, power_spectrum, threshold_factor=5.0
        )
        resonance_frequencies = [freq for freq, _ in peaks[:5]]

        # Estimate damping ratios (simplified)
        damping_ratios = self._estimate_damping_ratios(
            freqs, power_spectrum, resonance_frequencies
        )
        ...
        if high_freq_energy / total_energy > 0.3:
            anomalies.append(
                "High frequency content - possible structural damage or cracking"
            )
            health_score -= 0.3
        ...
        if structure_id in self.structural_baselines:
            ...
            if baseline.resonance_frequencies:
                ...
                freq_shift = abs(
                    resonance_frequencies[0]
                    - baseline.resonance_frequencies[0]
                )
                shift_percent = (
                    freq_shift / baseline.resonance_frequencies[0] * 100
                )

                if shift_percent > 5:
                    anomalies.append(
                        f"Resonance frequency shifted {shift_percent:.1f}% - possible stiffness change"
                    )
                    health_score -= 0.2
        ...
        if damping_ratios and np.mean(damping_ratios) < 0.01:
            anomalies.append(
                "Low damping - structure may be prone to resonance"
            )
            health_score -= 0.1
        ...
        if structure_id not in self.structural_baselines:
            ...
            alpha = 0.1
            old_baseline.health_score = (
                1 - alpha
            ) * old_baseline.health_score + alpha * health.health_score
            if health.resonance_frequencies:
                old_baseline.resonance_frequencies = (
                    health.resonance_frequencies
                )

        logger.info(
            f"Structural health for {structure_id}: score={health_score:.2f}, "
    ...
    def _estimate_damping_ratios(
        ...
        for res_freq in resonance_frequencies:
            ...
            while (
                ...
                and power_spectrum[right_idx] > half_power
            ):
                right_idx += 1
    ...
    def register_bearing(self, bearing_id: str, geometry: BearingGeometry):
        """
        ...
        logger.info(
            f"Registered bearing {bearing_id}: {geometry.num_balls} balls, "
