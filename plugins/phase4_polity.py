...
class Phase4PolityPlugin(PluginBase):
    ...
    def initialize(self, node):
        ...
        logger.info(
            f"Phase 4 Polity plugin initialized for node {node.node_id}"
        )
    ...
    def get_state_variables(self, node) -> Dict:
        ...
        return {
            ...
            "privilege_tier": (
                reputation.privilege_tier.value if reputation else 0
            ),
            "privilege_tier_name": (
                reputation.privilege_tier.name if reputation else "NEWBORN"
            ),
            "tasks_completed": reputation.tasks_completed if reputation else 0,
            "tasks_failed": reputation.tasks_failed if reputation else 0,
            "task_success_rate": (
                reputation.get_success_rate() if reputation else 0.0
            ),
            "squads_led": reputation.squads_led if reputation else 0,
            "strategies_proposed": (
                reputation.strategies_proposed if reputation else 0
            ),
            # Contract Net
            "active_contracts": active_tasks,
            "available_tasks": len(
                self._contract_net.get_available_tasks(agent_id)
            ),
            "contract_success_rate": success_rate,
            # Capabilities
            "can_join_tasks": (
                reputation.has_capability("join_task") if reputation else False
            ),
            "can_lead_squads": (
                reputation.has_capability("lead_squad")
                if reputation
                else False
            ),
            "can_propose_strategies": (
                reputation.has_capability("propose_strategy")
                if reputation
                else False
            ),
            # System status
    ...
    def get_available_actions(self, node) -> List[str]:
        ...
        if reputation:
            ...
            if reputation.has_capability("lead_squad"):
                actions.extend(
                    ["propose_task", "lead_squad", "request_resources"]
                )
    ...
    def award_reputation(
        ...
        if success:
            ...
            self.node_privilege_tier[node.node_id] = (
                reputation.privilege_tier.value
            )
    ...
    def propose_task(
        ...
        if not self.check_capability(node, "lead_squad"):
            logger.warning(
                f"Agent {agent_id} cannot propose tasks (insufficient privilege)"
            )
            return None
    ...
    def bid_on_task(
        ...
        if not self.check_capability(node, "join_task"):
            logger.warning(
                f"Agent {agent_id} cannot bid on tasks (insufficient privilege)"
            )
            return None
    ...
    def report_failure_for_analysis(
        self,
        failure_id: str,
    ...
    def propose_strategy_to_council(
        self, node, strategy_description: str, strategy_details: Dict
    ) -> bool:
        """
        ...
        if not self.check_capability(node, "propose_strategy"):
            logger.warning(
                f"Agent {agent_id} cannot propose strategies (requires EMERITUS tier)"
            )
            return False
    ...
    def evaluate_action_constitutionally(
        self,
        action_id: str,
    ...
    def get_summary(self) -> str:
        ...
        summary.append(
            f"   Tasks Announced: {contract_stats['total_tasks_announced']}"
        )
        summary.append(
            f"   Tasks Completed: {contract_stats['total_tasks_completed']}"
        )
        summary.append(
            f"   Completion Rate: {contract_stats['completion_rate']:.1%}"
        )
        summary.append(
            f"   Active Contracts: {contract_stats['active_tasks']}"
        )

        summary.append("\n3. COUNCIL OF PROFESSORS")
        summary.append("   Systemic Pathologist:")
        summary.append(
            f"     - Failures Analyzed: {gov_stats['pathologist']['total_failures_analyzed']}"
        )
        summary.append(
            f"     - Lessons Published: {gov_stats['pathologist']['total_lessons_published']}"
        )
        summary.append("   Strategic Immunologist:")
        summary.append(
            f"     - Vulnerabilities Tracked: {gov_stats['immunologist']['total_vulnerabilities_tracked']}"
        )
        summary.append(
            f"     - Antibodies Injected: {gov_stats['immunologist']['total_antibodies_injected']}"
        )
        summary.append("   Evolutionary Biologist:")
        summary.append(
            f"     - Assessments: {gov_stats['biologist']['total_assessments']}"
        )
        summary.append(
            f"     - Interventions: {gov_stats['biologist']['total_interventions']}"
        )

        summary.append("\n4. CONSTITUTIONAL FRAMEWORK")
        summary.append(
            f"   Actions Evaluated: {const_stats['total_actions_evaluated']}"
        )
        summary.append(
            f"   Compliance Rate: {const_stats['compliance_rate']:.1%}"
        )
        summary.append(
            f"   Violations Prevented: {const_stats['total_violations_prevented']}"
        )
        ...
        summary.append(
            f"   Commands Created: {bridge_stats['total_commands_created']}"
        )
        summary.append(
            f"   Commands Executed: {bridge_stats['total_commands_executed']}"
        )
        summary.append(f"   Success Rate: {bridge_stats['success_rate']:.1%}")
        summary.append(
            f"   Pending Approval: {bridge_stats['pending_approval']}"
        )
