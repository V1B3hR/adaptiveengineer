...
from plugins.advanced_sensors import (
    AdvancedSensorSuite,
    LiDARMode,
    CameraResolution,
)
...
def main():
    ...
    for i in range(3):
        ...
        drones.append(drone)
        logger.info(
            f"   Drone {i+1} positioned at ({x:.1f}, {y:.1f}, {z:.1f}m altitude)"
        )
    ...
    for i, drone in enumerate(drones):
        ...
        # RF scan for threats
        rf_results = drone.rf_sensor.analyze_spectrum(
            battery_level=drone.energy / 10.0
        )
        logger.info(
            f"      RF: {rf_results['signals_detected']} signals, "
        ...
        if acoustic_reading.signature_detected:
            logger.info(
                f"      Acoustic: {acoustic_reading.signature_detected} detected at "
        ...
        logger.info(
            f"      LiDAR: {len(lidar_scan.objects_detected)} objects at {lidar_scan.scan_rate_hz}Hz"
        )

        # Camera capture (surveillance mode)
        camera = drone.sensor_suite.capture_camera(
            CameraResolution.SURVEILLANCE
        )
        logger.info(
            f"      Camera: {camera.resolution[0]}x{camera.resolution[1]} @ {camera.frame_rate}fps"
        )
    ...
    if threats_detected:
        ...
        logger.info(
            "      Broadcasting to all drones for coordinated response"
        )
    else:
