...
def demonstrate_phase3():
    ...
    logger.info("=" * 70)
    logger.info(
        "PHASE 3 DEMONSTRATION: Learning, Evolution, Trust, and Consensus"
    )
    logger.info("=" * 70)
    ...
    for gen in range(10):
        stats = evolution.evolve_generation(
            StrategyType.DETECTION, evaluate_detection_strategy
        )
    ...
    logger.info(
        f"        - threshold: {best_strategy.parameters['threshold']:.3f}"
    )
    logger.info(
        f"        - sensitivity: {best_strategy.parameters['sensitivity']:.3f}"
    )
    logger.info(
        f"        - response_time: {best_strategy.parameters['response_time']:.3f}"
    )
    ...
    for i in range(50):
        ...
        result = learning.observe(
            BehaviorType.SERVICE, cpu_value, metadata={"metric": "cpu_usage"}
        )
    ...
    logger.info(
        f"        Auto-tuned thresholds: [{thresholds['lower_threshold']:.3f}, "
    ...
    for value in test_values:
        result = learning.observe(
            BehaviorType.SERVICE, value, metadata={"metric": "cpu_usage"}
        )
    ...
    logger.info(
        f"      ✓ Detected {anomalies_detected} anomalies from {len(test_values)} observations"
    )
    ...
    logger.info(
        f"      ✓ Trust network initialized with {len(trust_net.trust_network)} nodes"
    )
    ...
    logger.info(
        f"      ✓ Trusted nodes after positive interactions: {trusted}"
    )
    ...
    if trust_net.is_compromised(byzantine_node):
        logger.info(
            f"      ✓ Node {byzantine_node} detected and marked as compromised"
        )
    ...
    for nid, val in inputs.items():
        ...
        logger.info(
            f"        Node {nid}: {val:.2f} (trust={trust:.2f}) → {discounted:.2f}"
        )

    # Byzantine-resilient aggregation
    aggregated = trust_net.byzantine_resilient_aggregate(
        inputs, method="weighted_median"
    )
    logger.info(
        f"      ✓ Byzantine-resilient aggregated value: {aggregated:.3f}"
    )

    metrics = trust_net.get_byzantine_resilience_metrics()
    logger.info(f"      Network health: {metrics['network_health']:.2f}")
    logger.info(
        f"      Byzantine tolerance: {metrics['byzantine_tolerance']:.2f}"
    )
    ...
    for voter_id, vote_type, confidence in honest_votes + [byzantine_vote]:
        ...
        logger.info(
            f"      {vote_mark} Node {voter_id}: {vote_type} "
    ...
    if status["consensus_reached"]:
        ...
        logger.info(
            f"        Votes received: {status['votes_received']}/{status['quorum_size']}"
        )
        logger.info(
            "        → Byzantine node's vote was discounted by trust weighting"
        )
    ...
    if security_plugin and best_strategy:
        ...
        logger.info(
            f"      ✓ Applied evolved detection threshold: "
            f"{best_strategy.parameters['threshold']:.3f}"
        )
        logger.info(
            f"      ✓ Applied evolved sensitivity: "
    ...
    logger.info(
        f"      ✓ Learned from {stats['total_observations']} observations"
    )
    logger.info(
        f"      ✓ Auto-tuned thresholds {stats['threshold_adjustments']} times"
    )
    ...
    for voter_id, vote_type, confidence in [
        ...
        engine1.cast_vote(
            threshold_proposal_id, voter_id, vote_type, confidence
        )

    threshold_status = engine1.get_proposal_status(threshold_proposal_id)
    if threshold_status["consensus_reached"]:
        logger.info(
            f"      ✓ Consensus on threshold update: {threshold_status['consensus_result']}"
        )
        logger.info(
            f"        Confidence: {threshold_status['confidence']:.2f}"
        )
        logger.info(
            "        → System adapts thresholds collectively via consensus"
        )
    ...
    logger.info(
        f"      Best fitness: {best_strat.fitness:.4f}"
        if best_strat
        else "      Best fitness: N/A"
    )
    avg_fitness = (
        sum(s.fitness for s in population) / len(population)
        if population
        else 0
    )
    ...
    logger.info("\n   Adaptive Learning:")
    logger.info(
        f"      Total observations: {learning_stats['total_observations']}"
    )
    logger.info(
        f"      Anomalies detected: {learning_stats['anomalies_detected']}"
    )
    logger.info(
        f"      Threshold adjustments: {learning_stats['threshold_adjustments']}"
    )
    ...
    logger.info(
        f"      Compromised nodes: {trust_metrics['compromised_nodes']}"
    )
    logger.info(f"      Network health: {trust_metrics['network_health']:.2f}")
    logger.info(
        f"      Byzantine tolerance: {trust_metrics['byzantine_tolerance']:.2f}"
    )
    ...
    logger.info(
        f"      Finalized proposals: {consensus_stats['finalized_proposals']}"
    )
    logger.info(
        f"      Suspicious nodes detected: {consensus_stats['suspicious_nodes_count']}"
    )
    logger.info(
        f"      Trusted nodes: {consensus_stats['trusted_nodes_count']}"
    )
    ...
    logger.info(
        "       - Strategies improve through reproduction, variation, selection"
    )
    ...
    logger.info(
        "       - Consensus for root cause, attack validation, responses"
    )
    ...
    logger.info(
        f"      ✓ Deployed {sensory_cortex.get_total_sensor_count()} sensors"
    )
    ...
    logger.info(
        f"      ✓ Detected {len(inputs)} anomalies via multi-modal sensing:"
    )
    for inp in inputs:
        logger.info(
            f"        - {inp.sense_type.value}: {inp.anomaly_level.value} "
    ...
    for desc, features, strategy, outcome, time_taken in incidents:
        pattern_id = kg.recognize_pattern(
            desc, features, severity=max(features.values())
        )
        strategies_dict = {
            s.name: s.strategy_id for s in kg.solution_strategies.values()
        }
        ...
        if strategy_id:
            ...
            next_state = {
                "features": features,
                "resolved": outcome == OutcomeType.SUCCESS,
            }
            kg.record_incident(
                pattern_id, strategy_id, outcome, time_taken, state, next_state
            )

    kg_stats = kg.get_statistics()
    logger.info(
        f"      ✓ Learned from {kg_stats['total_incidents']} incidents:"
    )
    ...
    logger.info(
        f"        - RL experiences: {kg_stats['experiences_recorded']}"
    )
    logger.info(
        f"        - Root cause analyses: {kg_stats['root_cause_analyses']}"
    )
    ...
    for pattern in list(kg.problem_patterns.values())[:3]:
        ...
        if best:
            ...
            logger.info(
                f"      - '{pattern.description}' → '{strat.name}' (weight: {weight:.2f})"
            )
    ...
    response_id = immune_system.initiate_response(
        threat_sig, "node_005", ThreatLevel.HIGH
    )
    logger.info(f"      ✓ Immune response initiated: {response_id}")

    # Escalate to repair
    pattern_id = kg.recognize_pattern(
        "Malware Detected", threat_features, 0.85
    )
    ...
    squad_id = immune_system.deploy_reinforcements(
        "node_007",
        {"cpu_load": 0.92, "memory_usage": 0.88, "network_load": 0.75},
    )
    ...
    logger.info(
        f"        - Total responses: {immune_stats['total_responses']}"
    )
    ...
    for insight in insights[:5]:  # Show top 5
        logger.info(
            f"        - [{insight.insight_type.value}] {insight.description}"
        )
        logger.info(
            f"          Confidence: {insight.confidence:.2f}, Priority: {insight.priority:.2f}"
        )
    ...
    for hybrid in hybrid_strategies[:3]:
        ...
        logger.info(
            f"          Effectiveness: {hybrid.estimated_effectiveness:.2f}, "
    ...
    logger.info(
        f"        - Insights generated: {cog_stats['insights_generated']}"
    )
    logger.info(
        f"        - Hybrid strategies: {cog_stats['strategies_synthesized']}"
    )
    ...
    logger.info(
        "       - Genetic algorithms optimize detection/mitigation strategies"
    )
    logger.info(
        "       - Strategies evolve through reproduction, variation, selection"
    )
    ...
    logger.info(
        "       - Multi-modal sensing (sight, hearing, smell, taste, touch)"
    )
    logger.info(
        "       - Specialized sensor agents for different perspectives"
    )
