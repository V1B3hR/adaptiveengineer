...
def demonstrate_openness():
    ...
    logger.info("=" * 70)
    logger.info(
        "PHASE 5 DEMONSTRATION: Advanced Features, Openness, and Large-Scale"
    )
    logger.info("=" * 70)
    ...
    openness = OpennessEngine(
        node_id=1, adaptation_threshold=0.6, exploration_rate=0.2
    )
    ...
    for (
        ...
        logger.info(
            f"         Stability score: {env.get_stability_score():.2f}"
        )
        ...
        if should_adapt:
            ...
            if result.new_organization:
                logger.info(
                    f"         New organization: {result.new_organization.value}"
                )
    ...
    logger.info(
        f"       Environments encountered: {metrics['environments_encountered']}"
    )
    logger.info(
        f"       Adaptations performed: {metrics['adaptations_performed']}"
    )
    logger.info(
        f"       Success rate: {metrics['adaptation_success_rate']:.2%}"
    )
    logger.info(
        f"       Reorganizations: {metrics['reorganizations_performed']}"
    )
    ...
    if metrics["patterns_learned"] > 0:
        ...
        for pattern in patterns[:3]:  # Show first 3
            logger.info(
                f"         - {pattern['environment_type']}: "
                f"{pattern['encounter_count']} encounters"
            )
            if pattern["successful_strategies"]:
                logger.info(
                    f"           Successful: {pattern['successful_strategies']}"
                )
...
def demonstrate_human_loop():
    ...
    human_loop.approve_request(
        request2.request_id, approver="admin@example.com"
    )
    ...
    logger.info(
        f"       Approval requests: {metrics['approval_requests_made']}"
    )
    ...
    logger.info(
        f"       Explanations recorded: {metrics['explanations_recorded']}"
    )
...
def demonstrate_large_scale_simulation():
    ...
    adv_simulation = LargeScaleSimulation(
        node_count=200, mode=SimulationMode.ADVERSARIAL, enable_feedback=True
    )
    ...
    if feedback:
        ...
        if len(feedback) >= 2:
            ...
            logger.info(
                f"       Health improvement: {health_improvement:+.2f}"
            )
...
def demonstrate_integrated_phase5():
    ...
    if openness.should_adapt(env):
        ...
        if request.status in [
            ...
            result = openness.adapt_to_environment(env)
            logger.info(
                f"       Adaptation performed: {result.strategy_used.value}"
            )
            logger.info(f"       Success: {result.success}")
    ...
    logger.info(
        f"         Adaptations: {open_metrics['adaptations_performed']}"
    )
    logger.info(
        f"         Success rate: {open_metrics['adaptation_success_rate']:.2%}"
    )
    ...
    logger.info(
        f"         Approval requests: {hl_metrics['approval_requests_made']}"
    )
