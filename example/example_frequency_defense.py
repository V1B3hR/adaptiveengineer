...
def demonstrate_rf_analysis():
    ...
    results = analyzer.analyze_spectrum(duration_seconds=2.0, battery_level=1.0)
    
    logger.info(f"   ✓ Detected {results['signals_detected']} RF signals")
    logger.info(f"   ✓ Signal breakdown: {results['signal_breakdown']}")
    logger.info(f"   ✓ Spectrum utilization: {results['spectrum_utilization']:.2f}%")
    logger.info(f"   ✓ Threats detected: {results['threats']['total_threats']}")
    
    # Get statistics
    stats = analyzer.get_statistics()
    logger.info(f"   ✓ Total signals tracked: {stats['total_signals_detected']}")
...
def demonstrate_acoustic_analysis():
    ...
    analyzer = AcousticFrequencyAnalyzer(
        sample_rate_hz=44100,  # CD quality
        fft_size=2048
    )
    ...
    for event in events:
        logger.info(f"      - {event.signature.value} at {event.frequency_hz:.1f} Hz "
    ...
    logger.info(f"   ✓ Fundamental frequency: {diagnostic.fundamental_freq_hz:.1f} Hz")
...
def demonstrate_vibration_analysis():
    ...
    for event in events:
        if event.defect_type.value != "none":
            logger.info(f"      - {event.defect_type.value} at {event.frequency_hz:.2f} Hz "
                       f"(severity: {event.severity.value})")
    
    # Structural health monitoring
    logger.info("\n   Monitoring structural health...")
    health = analyzer.monitor_structural_health(
        structure_id="bridge_001",
        duration_seconds=10.0
    )
    
    logger.info(f"   ✓ Structural health score: {health.health_score:.2f}")
    logger.info(f"   ✓ Resonance frequencies: {[f'{f:.2f}Hz' for f in health.resonance_frequencies[:3]]}")
    if health.anomalies:
...
def demonstrate_network_analysis():
    ...
    for threat in c2_threats:
        ...
        logger.info(f"        Period: {threat.period_seconds:.1f}s, Confidence: {threat.confidence:.2f}")
...
def demonstrate_behavioral_analysis():
    ...
    logger.info(f"   ✓ Simulated {len(normal_behaviors)} normal behavior samples")
    ...
    for threat in threats:
        logger.info(f"      - {threat.malware_type.value}: {threat.description}")
        logger.info(f"        Severity: {threat.severity.value}, Confidence: {threat.confidence:.2f}")
...
def demonstrate_keystroke_analysis():
    ...
    logger.info(f"   ✓ Captured {len(user_keystrokes)} keystrokes for baseline")
    ...
    if signature:
        ...
        logger.info(f"      - Mean dwell time: {signature.mean_dwell_time*1000:.1f}ms")
        logger.info(f"      - Mean flight time: {signature.mean_flight_time*1000:.1f}ms")
    ...
    logger.info(f"      - Distance from baseline: {result.distance_from_baseline:.3f}")
...
def demonstrate_unified_intelligence():
    ...
    logger.info(f"   ✓ Overall threat score: {environment.overall_threat_score:.2f}")
    logger.info(f"   ✓ RF signals: {environment.rf_signals}, threats: {environment.rf_threats}")
    ...
    logger.info(f"   ✓ Active correlations: {len(report['active_correlations'])}")
    logger.info(f"   ✓ Recent correlated threats: {report['recent_correlated_threats']}")
...
def demonstrate_node_integration():
    ...
    if success:
        ...
        logger.info(f"   ✓ Threat score: {results.get('overall_threat_score', 0):.2f}")
        logger.info(f"   ✓ Active correlations: {results.get('active_correlations', [])}")
