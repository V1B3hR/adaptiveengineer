...
def create_infrastructure_graph() -> LivingGraph:
    ...
    logger.info(
        f"Created infrastructure graph with {len(graph.nodes)} nodes and {len(graph.edges)} edges"
    )
    return graph
...
def create_agent_population(graph: LivingGraph, num_agents: int = 12) -> list:
    ...
    for role, count in role_distribution:
        for _ in range(count):
            ...
            logger.info(
                f"Created agent {agent.node_id} with role {role.value} at node {node_id}"
            )
...
def demonstrate_cascading_failure_prevention(graph: LivingGraph, agents: list):
    ...
    logger.info(
        f"   ✓ Set high load on {len(node_ids)} nodes (precursor pattern)"
    )
    ...
    for scout in scout_agents[:2]:  # First 2 scouts
        ...
        logger.info(
            f"   ✓ Scout {scout['agent'].node_id} deposited threat pheromone at node {node_id}"
        )
    ...
    for agent_data in agents[:6]:  # Use half the population
        ...
        if plugin.execute_action(
            "predict_failure",
            {"metrics": current_metrics, "confidence_threshold": 0.5},
        ):
            predictions_made += 1

    logger.info(
        f"   ✓ {predictions_made} agents predicted potential cascading failure"
    )
    ...
    coordination_id = coordinator_plugin.swarm_manager.start_coordination(
        task_type="cascading_failure_prevention", target_node=str(node_ids[0])
    )

    # Agents join coordination
    agents_joined = 0
    for agent_data in agents:
        if agent_data["phase2_plugin"].execute_action(
            "join_coordination", {"coordination_id": coordination_id}
        ):
            agents_joined += 1

    logger.info(
        f"   ✓ Started coordination {coordination_id[:8]}... with {agents_joined} agents"
    )
    ...
    logger.info(
        f"   ✓ {len(healer_agents)} healers deployed suppression mechanisms"
    )
...
def demonstrate_adaptive_threat_mitigation(graph: LivingGraph, agents: list):
    ...
    logger.info(
        f"   ✓ Novel threat detected at node {threat_origin} (score: 0.9)"
    )
    ...
    logger.info(
        f"   ✓ {len(scout_agents)} scouts identified and broadcast threat signature"
    )
    ...
    logger.info(
        f"   ✓ Monitoring {len(adjacent_nodes)} adjacent nodes for spread"
    )

    # Simulate threat attempting to spread
    for adj_node in adjacent_nodes[:2]:
        graph.nodes[adj_node].attributes.active_threat_score = (
            0.3  # Attempted spread
        )
    ...
    logger.info(
        f"   ✓ {len(guardian_agents)} guardians deployed containment measures"
    )
    ...
    eradication_coord = coord_plugin.swarm_manager.start_coordination(
        task_type="threat_eradication", target_node=str(threat_origin)
    )

    # Multiple roles participate
    participating_agents = 0
    for agent_data in agents:
        if agent_data["role"] in [
            AgentRole.GUARDIAN,
            AgentRole.HEALER,
            AgentRole.SCOUT,
        ]:
            if agent_data["phase2_plugin"].execute_action(
                "join_coordination", {"coordination_id": eradication_coord}
            ):
...
def demonstrate_self_organizing_supply_chain(graph: LivingGraph, agents: list):
    ...
    logger.info(
        f"   ✓ Pipeline nodes: {' -> '.join(str(n) for n in chain_nodes)}"
    )
    ...
    for i, harvester in enumerate(harvester_agents):
        ...
        for j, node_id in enumerate(chain_nodes):
            plugin.execute_action(
                ...
                {
                    "type": (
                        PheromoneType.PROCESSING_NEEDED.value
                        if j < len(chain_nodes) - 1
    ...
    logger.info(
        f"   ✓ {len(harvester_agents)} harvesters deposited trail pheromones"
    )
    ...
    for harvester in harvester_agents[:1]:  # One harvester coordinates
        ...
        if plugin.execute_action(
            ...
            {"nodes": [str(n) for n in chain_nodes], "resource_type": "data"},
        ):
            chain_established += 1
    ...
    logger.info(
        f"   ✓ Total pheromone reinforcements: {stats['pheromone_reinforcements']}"
    )
    ...
    for harvester in harvester_agents[:1]:
        ...
        chain_id = (
            plugin.supply_chain_ids[0] if plugin.supply_chain_ids else None
        )
        if chain_id:
...
def demonstrate_emergent_statistics(agents: list):
    ...
    for agent_data in agents:
        ...
        total_pheromones += int(
            plugin.state_variables["pheromones_deposited"].value
        )
    ...
    logger.info(
        f"   Learned Precursors: {homeostasis_stats['learned_precursors']}"
    )
    logger.info(
        f"   Predictions Made: {homeostasis_stats['predictions_made']}"
    )
    logger.info(
        f"   Prediction Accuracy: {homeostasis_stats['prediction_accuracy']:.1%}"
    )
    logger.info(
        f"   Failures Prevented: {homeostasis_stats['failures_prevented']}"
    )
    ...
    logger.info(
        f"   Supply Chains Established: {swarm_stats['chains_established']}"
    )
    logger.info(f"   Chains Healed: {swarm_stats['chains_healed']}")
    logger.info(
        f"   Coordinated Actions: {swarm_stats['coordinated_actions']}"
    )
...
def main():
    ...
    logger.info(
        "  1. Cascading failure prevention (coordinated preemptive action)"
    )
    ...
    logger.info(
        "  ✓ Cascading failure prevention through collective prediction"
    )
    ...
    logger.info(
        "  ✓ Role specialization (Scouts, Harvesters, Guardians, etc.)"
    )
