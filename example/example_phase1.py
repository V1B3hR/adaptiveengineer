...
def demonstrate_plugin_system():
    ...
    logger.info("=" * 60)
    logger.info(
        "PHASE 1 DEMONSTRATION: Core Foundations & Emergency Self-Organization"
    )
    logger.info("=" * 60)
    ...
    logger.info(
        f"   ✓ {len(plugin_manager.plugins)} plugins registered and initialized"
    )
    ...
    for plugin_id, info in plugin_info.items():
        logger.info(
            f"     - {plugin_id} ({info['type']}): {len(info['state_variables'])} state variables, {len(info['actions'])} actions"
        )
    ...
    for var_name, value in al_state.items():
        ...
        logger.info(
            f"     - {var_name}: {value:.3f} (range: {var.min_value}-{var.max_value})"
        )
    ...
    for step in range(5):
        ...
        if step == 2:
            logger.info(
                f"   Step {step}: Normal operation - all systems stable"
            )
            logger.info(
                f"     Energy: {node.energy:.1f}, Anxiety: {node.anxiety:.1f}"
            )
            it_summary = it_plugin.get_health_summary()
            logger.info(
                f"     Service uptime: {it_summary['service_uptime']:.2%}, CPU: {it_summary['cpu_utilization']:.2%}"
            )
    ...
    logger.info(
        "   Simulating stress conditions (low energy + high threat)...\n"
    )
    ...
    logger.info(
        f"     - Defense posture: {security_summary['defense_posture']:.2f} (0=low, 1=high)"
    )
    logger.info(
        f"     - Anomaly detection: {security_summary['anomaly_score']:.2f}"
    )
    ...
    logger.info(
        f"     - Adaptation level: {al_plugin.state_variables['adaptation_level'].value:.2f}"
    )
    ...
    logger.info(
        f"     - Service uptime improved: {it_summary['service_uptime']:.2%} → {it_summary_after['service_uptime']:.2%}"
    )
    logger.info(
        f"     - Error rate cleared: {it_summary['error_rate']:.2%} → {it_summary_after['error_rate']:.2%}"
    )
    ...
    logger.info(
        f"     - Defense posture: {security_summary['defense_posture']:.2f} → {security_summary_after['defense_posture']:.2f}"
    )
