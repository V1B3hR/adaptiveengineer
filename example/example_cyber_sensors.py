...
def simulate_security_operations():
    ...
    logger.info(f"   📊 Malicious processes detected: {sum(len(t) for t in all_threats.values())}")
    
    for pid, threats in all_threats.items():
        for threat in threats:
            if threat.severity.value in ["malicious", "critical"]:
                logger.warning(f"      ⚠️  PID {pid}: {threat.malware_type.value} "
    ...
    if baseline:
        logger.info(f"   ✓ Baseline established (dwell: {baseline.mean_dwell_time*1000:.1f}ms)")
    ...
    logger.info(f"   📊 Auth result: {result.status.value} (confidence: {result.confidence:.2f})")
    ...
    total_network_threats = len(c2_threats) + len(port_scans) + len(covert_channels)
    ...
    logger.info(f"   Authentication events: {len(keystroke_analyzer.authentication_results)}")
    logger.info(f"   Security posture: {'CRITICAL' if total_network_threats + total_behavioral_threats > 3 else 'ELEVATED'}")
