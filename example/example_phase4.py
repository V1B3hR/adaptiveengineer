...
def demonstrate_reputation_and_meritocracy():
    ...
    for action_type, amount in [
        ("task_completion", 30), ("incident_resolution", 20),
        ("reliable_info", 10), ("task_completion", 40),
        ("task_completion", 50), ("squad_leadership", 30),
        ("task_completion", 100), ("strategy_proposal", 50)
    ]:
        ledger.record_reputation_change(agents[0], amount, action_type, "validator_1")
    
    # Agent 2: Progress to VETERAN
    for _ in range(10):
        ledger.record_reputation_change(agents[1], 60, "task_completion", "validator_2")
    
    # Agent 3: Reach TRUSTED_PEER
    for _ in range(5):
        ledger.record_reputation_change(agents[2], 25, "task_completion", "validator_1")
    
    # Agent 4: Some failures (negative reputation)
    for i in range(3):
        ledger.record_reputation_change(agents[3], 20, "task_completion", "validator_1")
        ledger.record_reputation_change(agents[3], -10, "task_completion", "validator_2")
    
    # Show progression
    logger.info("\n       Agent Progression:")
    for agent_id in agents:
        rep = ledger.get_reputation(agent_id)
        logger.info(f"       - {agent_id}: {rep.total_reputation:.0f} reputation "
    ...
    if ledger.verify_ledger_integrity():
        logger.info("       ✓ Ledger integrity verified - all signatures valid")
...
def demonstrate_contract_net_protocol():
    ...
    contract_net = ContractNetProtocol(
        default_bidding_time=5.0,  # Shorter for demo
    ...
    for i in range(6):
        ...
        ledger.record_reputation_change(agent_id, 50 * (i+1), "task_completion")
        agents[agent_id] = {
            'skills': all_skills[i],
            'energy': 50 + i*10
        }
    ...
    logger.info(f"       ✓ Task proposed: {task1.description} (reward={task1.reward})")
    ...
    logger.info(f"       ✓ Task proposed: {task2.description} (reward={task2.reward})")
    ...
    for agent_id, agent_data in agents.items():
        ...
        if task.requirements.is_qualified(
            agent_data['skills'], rep.total_reputation, 
            rep.privilege_tier.value, None, agent_data['energy']
        ):
            ...
            if bid:
                bids_submitted += 1
                logger.info(f"       - {agent_id} bid on Task 1 (score={bid.bid_score:.2f})")
        ...
        if task.requirements.is_qualified(
            ...
            if bid:
                bids_submitted += 1
                logger.info(f"       - {agent_id} bid on Task 2 (score={bid.bid_score:.2f})")
    ...
    for task_id in tasks:
        ...
        if task.awarded_to:
            # Report progress
            contract_net.report_progress(task_id, task.awarded_to, 0.5, "Task in progress")
            time.sleep(0.1)
            contract_net.report_progress(task_id, task.awarded_to, 0.9, "Nearly complete")
            time.sleep(0.1)
            # Complete task
            contract_net.complete_task(task_id, task.awarded_to)
            logger.info(f"       ✓ Task {task_id[-8:]} completed by {task.awarded_to}")
...
def demonstrate_council_of_professors():
    ...
    failures = [
        ("f1", "task_failure", {"insufficient_resources": True, "critical_service": True}),
        ("f2", "security_breach", {"weak_authentication": True, "widespread_impact": True}),
    ...
    for failure_id, failure_type, context in failures:
        ...
        logger.info(f"       ✓ Failure {failure_id} analyzed: {analysis.root_cause}")
        
        # Publish lesson learned
        lesson = council.pathologist.publish_lesson_learned(failure_id)
        if lesson:
            logger.info(f"         Lesson published: {lesson.what_not_to_do[:50]}...")
    
    logger.info(f"       Total failures analyzed: {len(failures)}")
    logger.info(f"       Lessons published: {council.pathologist.total_lessons_published}")
    ...
    if vuln.vuln_id in council.immunologist.chronic_vulnerabilities:
        logger.info("       ⚠ Chronic vulnerability identified - antibody injected")
        # Check if antibody was created
        antibodies = [a for a in council.immunologist.antibodies.values() 
                     if a.target_vulnerability == vuln.vuln_id]
        if antibodies:
            antibody = antibodies[0]
            logger.info(f"       ✓ Antibody type: {antibody.antibody_type.value}")
            # Deploy antibody
    ...
    logger.info(f"         - Diversity: {health.genetic_diversity:.2f} (threshold: 0.30)")
    logger.info(f"         - Stagnation: {health.stagnation_level:.2f} (threshold: 0.70)")
    ...
    logger.info(f"       Pathologist: {stats['pathologist']['total_failures_analyzed']} failures analyzed")
    logger.info(f"       Immunologist: {stats['immunologist']['total_antibodies_injected']} antibodies injected")
    logger.info(f"       Biologist: {stats['biologist']['total_interventions']} interventions performed")
...
def demonstrate_constitutional_framework():
    ...
    for i, test in enumerate(test_cases):
        ...
        logger.info(f"\n       {status}: {test['name']}")
        logger.info(f"         Compliance score: {evaluation.compliance_score:.2f}")
    ...
    logger.info(f"       Actions evaluated: {stats['total_actions_evaluated']}")
    logger.info(f"       Compliance rate: {stats['compliance_rate']:.1%}")
    logger.info(f"       Violations prevented: {stats['total_violations_prevented']}")
...
def demonstrate_real_world_bridge():
    ...
    for decision_type, decision_data in decisions:
        ...
        for cmd in commands:
            logger.info(f"       ✓ Command created: {cmd.system.value}:{cmd.operation}")
    ...
    for cmd in all_commands:
        ...
        if success:
            logger.info(f"       ✓ Command {cmd.command_id[-8:]} executed (DRY RUN)")
    
    # Show statistics
    logger.info("\n   10.5 Real-world bridge statistics...")
    stats = bridge.get_statistics()
    logger.info(f"       Commands created: {stats['total_commands_created']}")
    logger.info(f"       Commands executed: {stats['total_commands_executed']}")
...
def main():
    ...
    try:
        ...
        logger.info("\n" + "="*70)
        logger.info("PHASE 4: THE SENTIENT POLITY & GOVERNANCE LAYER - COMPLETE")
        logger.info("="*70)
        ...
        logger.info("  ✓ Reputation Ledger & Meritocratic Progression (4 Tiers)")
