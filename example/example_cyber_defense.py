...
def main():
    ...
    for i in range(5):
        ...
        defenders[i] = node
        print(f"✓ Defender Node {i}: Position {node.position}, Energy {node.energy:.2f}")
            ...
            for pattern, event in attacks:
                if event.success:
                    ...
                    if event.target_id in defenders:
                        ...
                        print(f"   💥 Attack on Node {event.target_id}: "
                        ...
                        if analysis['confidence'] > 0.5:
                            detection_count += 1
                            print(f"   🔍 Node {event.target_id} detected threat: "
                            ...
                            if countermeasure['constraints_satisfied']:
                                ...
                                if mitigation_success:
                                    mitigation_count += 1
                                    print(f"   🛡️  Countermeasure applied: "
                            ...
                            if shared > 0:
                                ...
                                print(f"   📡 Shared intelligence with {shared} nodes")
    ...
    print(f"\n🧬 Evolution:")
    print(f"   Threat Patterns in Library: {stats['threat_library']['total_patterns']}")
    print(f"   Evolution Generations: {stats['threat_library']['evolution_generations']}")
    print(f"   Average Attacker Fitness: {stats['attacker_stats']['avg_fitness']:.3f}")
    
    print(f"\n🛡️  Defender Metrics:")
    for defender_id, node in defenders.items():
        print(f"   Node {defender_id}: Energy={node.energy:.2f}/{node.max_energy:.2f}, "
            ...
            for pattern in patterns[:2]:  # Show top 2
                ...
                if best_cms:
                    for cm, effectiveness in best_cms:
                        print(f"      • {cm}: {effectiveness*100:.1f}% effective")
