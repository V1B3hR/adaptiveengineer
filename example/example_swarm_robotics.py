...
def main():
    ...
    for i in range(num_agents):
        ...
        node = AliveLoopNode(
            position=(np.random.uniform(5, 15), np.random.uniform(5, 15)),
            velocity=(0.0, 0.0),
            initial_energy=10.0,
            node_id=i,
        )
        ...
        agents[i] = node
        print(
            f"✓ Agent {i}: Position ({node.position[0]:.1f}, {node.position[1]:.1f})"
        )
    ...
    for step in range(30):
        ...
        if (step + 1) % 10 == 0:
            ...
            for node_id, node in agents.items():
                print(
                    f"  Agent {node_id}: Pos ({node.position[0]:.1f}, {node.position[1]:.1f}), "
    ...
    for task in tasks:
        ...
        print(
            f"✓ Added {task.task_id}: Priority {task.priority}, "
    ...
    print(
        f"✓ Detected {len(swarm_plugin.hardware_failures)} hardware failures"
    )
    ...
    for agent_id, node in agents.items():
        status = (
            "FAILED"
            if agent_id in swarm_plugin.hardware_failures
            else "ACTIVE"
        )
        print(
