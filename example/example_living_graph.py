...
def demonstrate_living_graph():
    ...
    # Add nodes
    web_server = graph.add_node(NodeType.SERVER, "WebServer-01", position=(0, 0))
    app_server = graph.add_node(NodeType.SERVER, "AppServer-01", position=(5, 0))
    db_server = graph.add_node(NodeType.DATABASE, "Database-01", position=(10, 0))
    load_balancer = graph.add_node(NodeType.SERVICE, "LoadBalancer", position=(0, 5))
    api_gateway = graph.add_node(NodeType.API_ENDPOINT, "APIGateway", position=(5, 5))
...
def demonstrate_advanced_communication():
    ...
    agent1 = AliveLoopNode(position=(0, 0), velocity=(0, 0), initial_energy=15.0, node_id=1)
    agent2 = AliveLoopNode(position=(2, 0), velocity=(0, 0), initial_energy=15.0, node_id=2)
    agent3 = AliveLoopNode(position=(4, 0), velocity=(0, 0), initial_energy=15.0, node_id=3)
    ...
    logger.info(f"   Message: SQL_INJECTION threat detected with 95% confidence")
    ...
    logger.info(f"   Agent 2 received {len(agent2.communication_protocol.signal_inbox)} signal(s)")
    logger.info(f"   Agent 3 received {len(agent3.communication_protocol.signal_inbox)} signal(s)")
    ...
    # Propagate gossip
    propagated = agent1.communication_protocol.propagate_gossip([agent2, agent3])
    ...
    for agent in [agent1, agent2, agent3]:
        ...
        logger.info(f"     - Gossip originated: {metrics['gossip_originated']}")
        logger.info(f"     - Gossip propagated: {metrics['gossip_propagated']}")
...
def demonstrate_agent_lifecycle(graph):
    ...
    logger.info(f"   Max population: {lifecycle_manager.birth_conditions.max_population}")
    logger.info(f"   Max lifespan: {lifecycle_manager.death_conditions.max_lifespan} time steps")
    
    # Spawn initial population
    logger.info("\n2. Spawning initial population...")
    
    # Find a healthy node to spawn on
    healthy_nodes = graph.query_nodes(health_status=HealthStatus.OK, max_stress=0.5)
    if healthy_nodes:
        ...
        for i in range(5):
            ...
            if agent:
                agents.append(agent)
                logger.info(f"   Spawned agent {agent.node_id} with energy {agent.energy:.1f}")
        
        logger.info(f"\n   Total agents spawned: {len(agents)}")
        logger.info(f"   Current population: {len(lifecycle_manager.living_agents)}")
    ...
    for t in range(50):
        ...
        if len(lifecycle_manager.living_agents) < len(agents):
            logger.info(f"   Time {t}: Population decreased to {len(lifecycle_manager.living_agents)}")
...
def main():
    ...
    logger.info("\n" + "=" * 70)
    logger.info("PHASE 1: THE PRIMORDIAL SOUP - ADVANCED FEATURES DEMONSTRATION")
    logger.info("=" * 70)
