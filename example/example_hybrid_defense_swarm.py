...
def main():
    ...
    for i in range(num_defenders):
        node = AliveLoopNode(
            position=(np.random.uniform(5, 20), np.random.uniform(5, 20)),
            velocity=(0.0, 0.0),
            initial_energy=10.0,
            node_id=i,
        )
        ...
        defenders[i] = node
        print(
            f"✓ Defense Agent {i}: Position ({node.position[0]:.1f}, {node.position[1]:.1f})"
        )
    ...
    print(
        f"✓ Protected center: ({protected_center[0]}, {protected_center[1]})"
    )
    ...
    env = AdversarialEnvironment(
        num_attackers=4, num_defenders=num_defenders, evolution_interval=25
    )
    ...
    visualize_hybrid_state(
        defenders, [protected_center], "Initial Defense Perimeter"
    )
    ...
    for step in range(50):
            ...
            for pattern, event in attacks:
                if event.success:
                    ...
                    if event.target_id in defenders:
                        ...
                        print(
                            f"   💥 Agent {event.target_id} under attack: "
                        ...
                        if analysis["confidence"] > 0.5:
                            detection_count += 1
                            print(
                                f"   🔍 Threat detected with confidence {analysis['confidence']:.2f}"
                            )
                            ...
                            if countermeasure["constraints_satisfied"]:
                                ...
                                if mitigation_success:
                                    print(
                                        f"   🛡️  Countermeasure: {countermeasure['actions'][0]}"
                                    )
                            ...
                            if shared > 0:
                                ...
                                print(
                                    f"   📡 Coordinated {shared} nearby defenders"
                                )

                                # Reposition swarm toward threat
                                # Update swarm center toward threat
                                threat_direction = (
                                    threat_loc - protected_center
                                )
        ...
        compromised = [
            nid for nid, node in defenders.items() if node.energy < 0.2
        ]
        if compromised:
    ...
    # Final visualization
    visualize_hybrid_state(
        defenders, threat_locations[-5:], "Final Defense State"
    )
    ...
    print(f"\n🧬 Threat Evolution:")
    print(
        f"   Threat Patterns: {env_stats['threat_library']['total_patterns']}"
    )
    print(
        f"   Evolution Generations: {env_stats['threat_library']['evolution_generations']}"
    )
    ...
    for agent_id, node in defenders.items():
        ...
        if node.energy < 0.1:
            status = "CRITICAL"
        print(
            f"   Agent {agent_id}: Energy={node.energy:.2f}, "
