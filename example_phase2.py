...
def demonstrate_phase2():
    ...
    logger.info("=" * 70)
    logger.info("PHASE 2 DEMONSTRATION: Communication, Security, and Incident Memory")
    logger.info("=" * 70)
    ...
    logger.info(f"   ✓ Initialized 4 plugins per node (IT, Security, AL, Communication)")
    ...
    logger.info(f"     ✓ Sent ALERT message (ID: {alert_msg.message_id[:8]}...)")
    ...
    logger.info(f"     ✓ Sent ANOMALY message (ID: {anomaly_msg.message_id[:8]}...)")
    ...
    logger.info(f"     ✓ Sent REMEDIATION message (ID: {remediation_msg.message_id[:8]}...)")
    ...
    logger.info(f"     ✓ Sent TRUST_UPDATE message (ID: {trust_msg.message_id[:8]}...)")
    ...
    logger.info(f"     ✓ Found {len(trace_messages)} messages in trace {alert_msg.trace_id[:8]}...")
    ...
    logger.info(f"     ✓ Published security event to {stream_count} streams (SIEM, AIOps)")
    ...
    logger.info(f"     ✓ Published operational event to {stream_count} streams")
    ...
    for i in range(5):
        ...
        if i == 0:
            ...
            logger.info(f"     ⚠ Pattern detected after {pattern.occurrence_count} occurrences!")
    ...
    logger.info(f"     ✓ Queried {len(recent_incidents)} recent alert incidents")
    ...
    logger.info(f"     Incidents recorded: {comm1_stats['incidents_recorded']}")
    ...
    logger.info(f"     Total incidents processed: {memory_stats['total_incidents_processed']}")
    logger.info(f"     Total patterns identified: {memory_stats['total_patterns_identified']}")
    logger.info(f"     Stored incidents: {memory_stats['store_stats']['total_incidents']}")
    
    if memory_stats['top_patterns']:
        logger.info(f"\n   Top Patterns:")
        for pattern in memory_stats['top_patterns'][:3]:
            logger.info(f"     - {pattern['pattern_id']}: {pattern['count']} occurrences "
    ...
    logger.info("      - Rich message types (alert, event, anomaly, remediation, trust_update)")
